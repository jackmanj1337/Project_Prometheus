...
func _on_campaign_rule_flipped(rule_id: String, value: Variant, reason: String,
		revert_scope: String) -> void:
	_generation += 1
	var generation := _generation
	var scope := "this map" if revert_scope == "end_of_map" else "campaign"
	_label.text = "Rule changed — %s: %s (%s; %s)" % [
		rule_id.capitalize(), str(value), reason, scope]
	show()
