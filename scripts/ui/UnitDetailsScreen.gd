...
var _description_stick_direction := 0
var _description_stick_timer := 0.0
...
func _show_entry(category: String, key: String, title: String) -> void:
	...
	if category == "class":
		...
		_info_desc.text = BBCode.escape(MoreInfoContent.describe(category, key))
...
func _scroll_description(direction: int) -> bool:
	...
	bar.value = clampf(
		before + direction * _DESCRIPTION_SCROLL_STEP, 0.0, maxf(0.0, bar.max_value - bar.page)
	)
	_scroll_hint.show()
...
func _inventory_description(key: String) -> String:
	...
	return BBCode.escape(MoreInfoContent.describe("inventory", kind))
...
func _weapon_info_text(weapon_id: String) -> String:
	...
	if w == null:
		return BBCode.escape(MoreInfoContent.describe("inventory", "weapon"))
	var lines: Array[String] = [BBCode.escape(MoreInfoContent.describe("inventory", "weapon")), ""]
	...
	lines.append(
		(
			"Rank %s (%s)   Uses %s"
			% [
				BBCode.escape(w.required_rank),
				BBCode.escape(w.combat_family.capitalize()),
				uses_text
			]
		)
	)
	if not w.effect_tags.is_empty():
		var safe_effects: Array[String] = []
		for tag in w.effect_tags:
			safe_effects.append(BBCode.escape(String(tag)))
		lines.append("Effects: " + ", ".join(safe_effects))
	return "\n".join(lines)
...
func _item_info_text(item_id: String) -> String:
	...
	if it != null and it.description != "":
		return BBCode.escape(it.description)
	return BBCode.escape(MoreInfoContent.describe("inventory", "item"))
...
func _class_description(class_id: String) -> String:
	...
	if not families.is_empty():
		...
		for f in families:
			fam_labels.append(BBCode.escape(String(f).capitalize()))
	...
	if not cd.skill_unlocks.is_empty():
		...
		levels.sort()
		for lv in levels:
			unlock_parts.append("Lv%d %s" % [int(lv), BBCode.escape(String(cd.skill_unlocks[lv]))])
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		if event.physical_keycode == KEY_PAGEUP or event.keycode == KEY_PAGEUP:
			if _scroll_description(-1):
				get_viewport().set_input_as_handled()
			return
		if event.physical_keycode == KEY_PAGEDOWN or event.keycode == KEY_PAGEDOWN:
			if _scroll_description(1):
				get_viewport().set_input_as_handled()
			return
