...
func validate(request: RefCounted, context: RefCounted):
	...
	if _registry == null or not _registry.has_entry("action_primitives", request.primitive_id):
		return ActionResultScript.failure("unknown_primitive", "Unknown action primitive '%s'." % request.primitive_id)
	...
	if not _handlers.has(handler_id):
		return ActionResultScript.failure("unknown_handler", "No runtime handler for '%s'." % handler_id)
	for subject_id in entry.subjects:
		if not context.subjects.has(subject_id) or context.subjects[subject_id] == null:
			return ActionResultScript.failure("missing_subject", "Required subject '%s' is missing." % subject_id,
				{"subject": subject_id})
	for param_id in entry.params_schema:
		...
		if not request.params.has(param_id):
			if bool(spec.get("required", false)):
				return ActionResultScript.failure("missing_param", "Required parameter '%s' is missing." % param_id,
					{"parameter": param_id})
			continue
		if not _matches_type(request.params[param_id], String(spec.get("type", ""))):
			return ActionResultScript.failure("invalid_param_type", "Parameter '%s' must be %s." % [param_id, spec.type],
				{"parameter": param_id, "expected": spec.type})
		if bool(spec.get("non_empty", false)) and String(request.params[param_id]).strip_edges() == "":
			return ActionResultScript.failure("invalid_param_value", "Parameter '%s' cannot be empty." % param_id,
				{"parameter": param_id})
		if bool(spec.get("non_zero", false)) and int(request.params[param_id]) == 0:
			return ActionResultScript.failure("invalid_param_value", "Parameter '%s' cannot be zero." % param_id,
				{"parameter": param_id})
	var target: Variant = context.subjects.get("target")
	if handler_id == "apply_active_modifier" and (not target is Node or not target.has_method("add_modifier")):
		return ActionResultScript.failure("invalid_subject", "Target cannot receive active modifiers.", {"subject": "target"})
