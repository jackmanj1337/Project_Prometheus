...
func load_entries(root: String) -> Array[String]:
	...
	for path in ResourceManifest.load_paths(root):
		...
		if resource.family != "item_effects":
			...
			continue
		errors.append_array(register_effect(resource.id,
...
func register_effect(effect_id: String, validation_handler: String,
		runtime_handler: String, preview_mode: String = "direct") -> Array[String]:
	var errors: Array[String] = []
	...
	if not _validators.has(validation_handler):
		errors.append("ItemEffectRegistry: effect '%s' has unknown validation handler '%s'" % [
	...
	if errors.is_empty():
		_entries[effect_id] = {"validation_handler": validation_handler,
			"runtime_handler": runtime_handler, "preview_mode": preview_mode}
	return errors
...
func register_runtime_handler(handler_id: String, can_apply: Callable,
		preview: Callable, commit: Callable) -> Array[String]:
	if handler_id.strip_edges().is_empty() or not can_apply.is_valid() \
			or not preview.is_valid() or not commit.is_valid():
...
func validate_item(item: ItemData, classes: Dictionary) -> Array[String]:
	...
	if not _entries.has(item.effect_id):
		errors.append("DataManager: item '%s' effect_id '%s' is not registered" % [item.id, item.effect_id])
		return errors
...
func _validate_amount(item: ItemData, _classes: Dictionary) -> Array[String]:
	if not (item.effect_params.get("amount", null) is int) \
...
func _validate_class_refs(item: ItemData, classes: Dictionary) -> Array[String]:
	...
	for class_id in item.effect_params.get("allowed_classes", []):
		if not classes.has(String(class_id)):
			errors.append("DataManager: item '%s' allowed_classes '%s' not found" % [item.id, String(class_id)])
	var groups := {}
	...
	for group_id in item.effect_params.get("allowed_class_groups", []):
		if not groups.has(String(group_id)):
			errors.append("DataManager: item '%s' allowed_class_groups '%s' not found" % [item.id, String(group_id)])
	return errors
...
func _validate_stat_buff(item: ItemData, _classes: Dictionary) -> Array[String]:
	...
	if not (item.effect_params.get("delta", null) is int) or int(item.effect_params.get("delta", 0)) == 0:
