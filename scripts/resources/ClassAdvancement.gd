...
static func commit_state(state: Dictionary, resolution: Dictionary, confirmed: bool) -> bool:
	if not confirmed or not bool(resolution.get("valid", false)):
		return false
	state["class_id"] = String(resolution.get("destination_class_id", ""))
	state["class_variant_id"] = _nullable_id(resolution.get("class_variant_id", ""))
	state["advancement_edge_id"] = String(resolution.get("edge_id", ""))
	state["advancement_edge_variant_id"] = _nullable_id(resolution.get("edge_variant_id", ""))
	for stat in resolution.get("stat_gains", {}):
		state[stat] = int(state.get(stat, 0)) + int(resolution["stat_gains"][stat])
	var wexp: Dictionary = state.get("weapon_wexp", {}).duplicate(true)
