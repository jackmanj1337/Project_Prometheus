...
func validate_document(
	...
	if not document is Dictionary:
		errors.append(_error("type_mismatch", "$", "Document must be an object."))
		return errors
	...
	for field: Variant in fields:
		...
		if not properties.has(field_name):
			...
			continue
		_validate_value(
			document[field_name], properties[field_name], "$.%s" % field_name, sources, errors
		)
	return errors
...
func _validate_value(
	...
	match String(field_schema.get("type", "")):
		"string":
