...
static func validate(
	plan: Dictionary, roster: Array[UnitData], node: CampaignNode, start_tiles: Array[Vector2i]
) -> Array[String]:
	...
	if plan.size() > start_tiles.size():
		errors.append(
			(
				"DeploymentPlan: plan deploys %d units but the map has only %d start tiles"
	...
	for key in plan:
		...
		if not start_tiles.has(tile):
			errors.append(
				(
					"DeploymentPlan: unit '%s' is placed on %s, which is not a player start tile"
					% [unit_id, tile]
				)
			)
		if tile_owner.has(tile):
			errors.append(
				(
					"DeploymentPlan: units '%s' and '%s' are both placed on %s"
...
static func _node_constraint_errors(
	plan: Dictionary, party: Dictionary, node: CampaignNode
) -> Array[String]:
	var errors: Array[String] = []
	...
	if node.deployment_cap != -1 and plan.size() > node.deployment_cap:
		errors.append(
			(
				"DeploymentPlan: node '%s' caps deployment at %d, plan deploys %d"
	...
	for unit_id in node.required_units:
		...
		if not party.has(unit_id):
			...
			errors.append(
				(
					"DeploymentPlan: node '%s' requires unit '%s', which is not in the party"
		...
		if (party[unit_id] as UnitData).is_incapacitated:
			...
			continue
		errors.append(
			"DeploymentPlan: node '%s' requires unit '%s' to deploy" % [node.node_id, unit_id]
		)
