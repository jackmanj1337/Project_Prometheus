...
func _init() -> void:
	...
	var source := {
		"author_id": "author_a", "campaign_id": "short_campaign",
		"campaign_version": "1.0.0", "compatible_status_sources": [],
	}
	...
	var exported: Dictionary = store.export_completion(source, state,
	...
	var sequel := {
		"author_id": "author_a", "campaign_id": "sequel",
	...
	var foreign := {
		"author_id": "author_b", "campaign_id": "foreign",
		"campaign_version": "1.0.0", "compatible_status_sources": [],
	}
	_check(same_campaign.size() == 1 and sequel_matches.size() == 1
	...
	_check(store.import_into(sequel_matches[0]["record"], sequel, imported)
	...
	_check(cm.import_carry_forward_facts(imported.carry_forward_facts)
	...
	_check(clean_state.carry_forward_facts.is_empty()
			and clean_state.imported_record_ref.is_empty(),
		"choosing None leaves a clean mutable campaign state")
	_check(not store.import_into(exported["record"], foreign, clean_state)
	...
	_check(not store.import_into(tampered, source, unchanged, true)
