...
func _init() -> void:
	...
	if empty.ok and empty.to_tile == Vector2i(2, 2):
		print("OK  empty tile succeeds"); passed += 1
	else:
		print("FAIL empty tile: %s" % empty.failure_reason); failed += 1
	...
	if not blocked.ok and blocked.failure_reason == "occupied":
		print("OK  require_empty rejects occupied tile"); passed += 1
	else:
		print("FAIL occupied tile: ok=%s reason=%s" % [blocked.ok, blocked.failure_reason]); failed += 1
	...
	if nearest.ok and nearest.fallback_used and nearest.to_tile == Vector2i(2, 1):
		print("OK  nearest_free tie-break is distance, y, x"); passed += 1
	else:
		print("FAIL nearest fallback: ok=%s tile=%s" % [nearest.ok, nearest.to_tile]); failed += 1
	...
	if not skipped.ok and skipped.skipped and service.delayed_requests.is_empty():
		print("OK  skip reports without mutation"); passed += 1
	else:
		print("FAIL skip result"); failed += 1
	...
	if not delayed.ok and delayed.queued and service.delayed_requests.size() == 1:
		print("OK  delay queues without placement"); passed += 1
	else:
		print("FAIL delay result"); failed += 1
	...
	if not reserved.ok and reserved.failure_reason == "not_implemented":
		print("OK  reserved policy fails structurally"); passed += 1
	else:
		print("FAIL reserved policy: %s" % reserved.failure_reason); failed += 1
	...
	if not unknown.ok and unknown.failure_reason == "unknown_policy":
		print("OK  unknown policy fails validation"); passed += 1
	else:
		print("FAIL unknown policy: %s" % unknown.failure_reason); failed += 1
