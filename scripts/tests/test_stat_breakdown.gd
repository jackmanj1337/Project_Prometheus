...
func _init() -> void:
	...
	if StatBreakdown.label_for_stat("strength") == "Str":
		print("OK  label_for_stat known")
		passed += 1
	else:
		print("FAIL label_for_stat known")
		failed += 1
	if StatBreakdown.label_for_stat("aura") == "Aura":
		print("OK  label_for_stat fallback capitalises unknown stat")
		passed += 1
	else:
	...
	if (
		StatBreakdown.label_for_source("tonic") == "Tonic"
		and StatBreakdown.label_for_source("mystery_skill") == "mystery_skill"
	):
		print("OK  label_for_source known + fallback to raw id")
		passed += 1
	else:
		print("FAIL label_for_source")
		failed += 1

	# ---- format_signed always includes sign -------------------------------
	if (
		StatBreakdown.format_signed(2) == "+2"
		and StatBreakdown.format_signed(-1) == "-1"
		and StatBreakdown.format_signed(0) == "+0"
	):
		print("OK  format_signed always includes sign")
		passed += 1
	else:
		print("FAIL format_signed")
		failed += 1
	...
	if durations_ok:
		print("OK  format_duration covers turn/map_turn/combat/permanent")
		passed += 1
	else:
		print("FAIL format_duration")
		failed += 1
	...
	if vocab_ok:
		print("OK  format_duration renders the V021-09 display vocabulary")
		passed += 1
	else:
		print("FAIL V021-09 vocabulary labels")
		failed += 1
	...
	if coverage_ok:
		print("OK  format_duration handles every VALID_DURATION_TYPES entry (DoD#2)")
		passed += 1
	else:
		print("FAIL VALID_DURATION_TYPES coverage")
		failed += 1
	...
	if (
		...
		and empty["label"] == "Str"
		and empty["base"] == 0
		and empty["effective"] == 0
		and empty["total_delta"] == 0
		and empty["mods"] == []
	):
		print("OK  build(null) returns safe empty shape")
		passed += 1
	else:
		print("FAIL build(null): %s" % empty)
		failed += 1
	...
	if (
		no_mods["base"] == 9
		and no_mods["effective"] == 9
		and no_mods["total_delta"] == 0
		and (no_mods["mods"] as Array).is_empty()
	):
		print("OK  build returns base==effective with no mods")
		passed += 1
	else:
		print("FAIL build no-mods: %s" % no_mods)
		failed += 1
	...
	if single_ok:
		print("OK  build filters by stat, includes friendly source + duration")
		passed += 1
	else:
	...
	d.active_modifiers = [
		{"stat": "strength", "delta": 2, "source": "tonic", "duration": 1, "duration_type": "turn"},
		{"stat": "strength", "delta": 2, "source": "tonic", "duration": 3, "duration_type": "turn"},
	]
	...
	if (
		not unknown_mods.is_empty()
		and (unknown_mods[0] as Dictionary)["source_label"] == "mystery_skill"
	):
		print("OK  unknown source label falls back to raw id")
		passed += 1
	else:
		print("FAIL unknown-source fallback: %s" % unknown)
		failed += 1
	...
	if deco_ok:
		print("OK  build decomposes base into personal + class and reads the cap")
		passed += 1
	else:
		print("FAIL decomposition: %s" % deco)
		failed += 1
	...
	if uncapped["cap_state"] == "uncapped":
		print("OK  stats outside STAT_KEYS report cap_state 'uncapped'")
		passed += 1
	else:
		print("FAIL uncapped: %s" % uncapped)
		failed += 1
	...
	if missing["cap_state"] == "missing" and missing["cap"] == StatBreakdown.CAP_MISSING:
		print("OK  capped stat with no cap entry reports cap_state 'missing'")
		passed += 1
	else:
		print("FAIL missing-cap: %s" % missing)
		failed += 1
	...
	if unknown_cap["cap_state"] == "unknown" and unknown_cap["personal_base"] == int(d.strength):
		print("OK  no class_data reports cap_state 'unknown'")
		passed += 1
	else:
		print("FAIL unknown-cap: %s" % unknown_cap)
		failed += 1
	...
	if below["personal_base"] == 0 and below["class_base"] == 6:
		print("OK  personal_base clamps at 0 when stored stat is below class base")
		passed += 1
	else:
		print("FAIL personal clamp: %s" % below)
		failed += 1
	...
	var merged_ok: bool = (
		...
		and StatBreakdown.label_for_source("pair_up:m1:strength") == "Pair Up"
	)  # prefix label
	if merged_ok:
		print("OK  extra_mods merge into mods and raise effective_display only")
		passed += 1
	else:
		print("FAIL extra_mods merge: %s" % merged)
		failed += 1
