...
func _init() -> void:
	...
	if adapted.valid and adapted.campaigns.has("fixture") \
			and adapted.map_registry["map_01"]["map_data_path"] == map_uri \
			and adapted.map_registry["map_01"]["roster_policy"] == "campaign_pack_roster":
		print("OK  manifest/catalogue becomes a stable package-scoped launch graph"); passed += 1
	else:
		print("FAIL runtime graph: %s" % [adapted.errors]); failed += 1

	var map: MapData = adapted.maps.get("map_01")
	var roster: Array = adapted.rosters.get("heroes", [])
	if map != null and map.grid == ["..."] and map.player_start_tiles == [Vector2i(0, 0)] \
			and roster.size() == 1 and roster[0] is UnitData \
			and roster[0].hp == 20 and roster[0].movement == 5:
		print("OK  JSON map, class bases, and roster adapt to engine Resource types"); passed += 1
	else:
		print("FAIL runtime resources: grid=%s starts=%s hp=%s move=%s" % [
			map.grid if map else null, map.player_start_tiles if map else null,
			roster[0].hp if not roster.is_empty() else null,
			roster[0].movement if not roster.is_empty() else null]); failed += 1

	var dm := DataManagerScript.new()
	if dm.select_tier2_campaign_source(pack, ROOT, "1.0") \
		...
		print("OK  DataManager atomically selects and resolves the Tier-2 source"); passed += 1
	else:
		print("FAIL DataManager Tier-2 selection"); failed += 1
	dm.free()
	...
	if not rejected and live_dm.active_package_identity() == before \
			and live_dm.has_campaign("fixture"):
		print("OK  invalid activation preserves the previously selected registries"); passed += 1
	else:
		print("FAIL atomic selection preservation"); failed += 1
	live_dm.free()
...
func _write_pack(root: String, base_hp: int = 20) -> void:
	var files := {
		...
		"data/roster.json": {"units": [{
			"unit_id": "hero", "unit_name": "Hero", "class_id": "fixture_class",
		}]},
		"data/class.json": {
			"id": "fixture_class", "display_name": "Fixture",
			"base_hp": base_hp, "base_movement": 5,
		},
	}
