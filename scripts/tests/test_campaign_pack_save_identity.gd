...
func _run() -> void:
	...
	if dm == null or gs == null or cm == null:
		print("FAIL required autoloads unavailable"); quit(1); return
	...
	if selected and save.campaign["package_id"] == ROOT \
			and save.campaign["package_version"] == "1.0":
		print("OK  between-map campaign save captures exact package identity"); passed += 1
	else:
		print("FAIL campaign package capture: %s" % [save.campaign if save else null]); failed += 1

	dm.call("select_campaign_source", "res://data")
	cm.call("end_campaign")
	if gs.call("configure_campaign_resume", save) \
			and dm.call("active_package_identity")["package_id"] == ROOT \
			and cm.get("active_campaign_id") == "fixture":
		print("OK  campaign load activates installed package before restoring ids"); passed += 1
	else:
		print("FAIL package-aware campaign restore"); failed += 1
	...
	if suspend.campaign["package_id"] == ROOT and suspend.campaign["package_version"] == "1.0":
		print("OK  mid-map document carries the same package identity"); passed += 1
	else:
		print("FAIL suspend package capture: %s" % [suspend.campaign]); failed += 1
	...
	if not gs.call("configure_campaign_resume", malformed):
		print("OK  incomplete package identity is rejected"); passed += 1
	else:
		print("FAIL incomplete package identity accepted"); failed += 1
...
func _write_pack(root: String) -> void:
	var files := {
		...
		"data/campaign.json": {"campaign_id": "fixture", "label": "Fixture",
