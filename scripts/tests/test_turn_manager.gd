...
func _init() -> void:
	...
	if gs.party_gold == 100 and gs.party_items == ["vulnerary"] \
			and reward_receipt == {"gold_earned": 75, "total_gold": 100,
				"items_awarded": ["vulnerary"]}:
	...
	# Zero is an explicit receipt, never stale data from a prior map.
	...
	if zero_receipt.gold_earned == 0 and zero_receipt.total_gold == 100:
		print("OK  zero-gold victory produces an honest committed receipt"); passed += 1
	else:
		print("FAIL zero reward receipt: %s" % zero_receipt); failed += 1
