...
func _init() -> void:
	...
	var source := {
		...
		"campaign":
		{
			...
			"cleared_nodes": [],
			"rules": {},
			"protected_fields": ["party.resources.reputation"],
		},
		"party": {"resources": {"reputation": 7}},
	}
	var stamped: Dictionary = Integrity.stamp(source)
	if Integrity.verify(stamped).is_empty():
		print("OK  freshly stamped save verifies")
		passed += 1
	else:
		print("FAIL freshly stamped save warned")
		failed += 1
	...
	if ordinary_warnings == [Integrity.PAYLOAD_WARNING]:
		print("OK  ordinary edits warn only on the whole payload")
		passed += 1
	else:
		print("FAIL ordinary warning set: %s" % [ordinary_warnings])
		failed += 1
	...
	if (
		Integrity.PAYLOAD_WARNING in protected_warnings
		and Integrity.PROTECTED_WARNING in protected_warnings
	):
		print("OK  authored dotted paths participate in the protected hash")
		passed += 1
	else:
		print("FAIL authored protected warning set: %s" % [protected_warnings])
		failed += 1
