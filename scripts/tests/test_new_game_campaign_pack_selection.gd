...
func _run() -> void:
	...
	for index in screen._run_options.size():
		if (
			screen._run_options[index].get("package_id", "") == ROOT
	...
	if (
		installed_index >= 0
		and "Selector Campaign" in run_opt.get_item_text(installed_index)
		and ROOT in run_opt.get_item_text(installed_index)
	):
		print("OK  New Game appends validated installed campaigns with pack identity")
		passed += 1
	else:
		print("FAIL installed selector rows: count=%d" % run_opt.item_count)
		failed += 1

	var dm: Node = root.get_node("DataManager")
	var run: Dictionary = screen._run_options[installed_index]
	if (
		screen._activate_run_source(run)
		and dm.call("active_package_identity")["package_id"] == ROOT
		and dm.call("has_campaign", "selector_campaign")
	):
		print("OK  selected row activates the exact Tier-2 source before start")
		passed += 1
	else:
	...
	gs.call(
		"configure_next_map",
		params["map_data_path"],
		params["roster_policy"],
		params["roster_source"]
	)
	if (
		params["roster_policy"] == "campaign_pack_roster"
		and cm.call("_apply_roster_policy", gs, params["roster_policy"], params["roster_source"])
		and gs.call("is_roster_ready_for_launch")
	):
		print("OK  selected campaign resolves its package map and playable roster")
		passed += 1
	else:
		print("FAIL package campaign launch resources")
		failed += 1
...
func _write_pack(root_path: String) -> void:
	var files := {
		...
		"data/campaign.json":
		{
			"campaign_id": "selector_campaign",
			"label": "Selector Campaign",
			"start_node_id": "start",
