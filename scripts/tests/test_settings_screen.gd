...
func _init() -> void:
	...
	if packed == null:
		print("FAIL could not load SettingsScreen.tscn")
		quit(1)
		return
	...
	if screen.get_node_or_null("Dimmer") != null:
		print("OK  Dimmer node present (#1 opacity)")
		passed += 1
	else:
		print("FAIL no Dimmer node")
		failed += 1
	...
	var input_prompt_label := (
		screen.get_node_or_null("Panel/ScrollContainer/Margin/VBox/HBoxInputMode/Label") as Label
	)
	if input_prompt_label != null and input_prompt_label.text == "Input Prompts":
		...
		print(
			(
				"FAIL input prompt label: %s"
				% [input_prompt_label.text if input_prompt_label != null else "<none>"]
			)
		)
		failed += 1

	var menu_scale_title := screen.get_node_or_null(
		"Panel/ScrollContainer/Margin/VBox/HBoxUIScale/LabelUIScaleTitle"
	)
	if menu_scale_title != null and String(menu_scale_title.get("text")) == "Menu Scale":
		print("OK  display scale row is labeled Menu Scale")
		passed += 1
	else:
	...
	screen.show()
	var scale_slider: Control = screen.get_node_or_null(
		"Panel/ScrollContainer/Margin/VBox/HBoxUIScale/SliderUIScale"
	)
	...
	if scale_slider != null and panel_for_slider != null:
		for factor in [0.5, 1.0, 2.0]:
			...
			await process_frame
			slider_offsets.append(
				(
					scale_slider.get_global_rect().position.x
					- panel_for_slider.get_global_rect().position.x
				)
			)
	var slider_stable := (
		slider_offsets.size() == 3
	...
	var scale_row: Control = screen.get_node_or_null(
		"Panel/ScrollContainer/Margin/VBox/HBoxUIScale"
	)
	...
	if scroll != null and scroll.horizontal_scroll_mode == ScrollContainer.SCROLL_MODE_DISABLED:
		print("OK  V025-01b settings horizontal scroll is disabled")
		passed += 1
	else:
		print(
			(
				"FAIL settings h-scroll not disabled: mode=%s"
				% [scroll.horizontal_scroll_mode if scroll != null else "<no scroll>"]
			)
		)
		failed += 1
	...
	if monotonic_down and monotonic_up and bounded_steps:
		...
		print(
			(
				"FAIL Settings scroll: down=%s up=%s bounded=%s"
	...
	if root.get_node_or_null("SettingsManager") != null and scale_slider != null:
		...
		if previewed_not_applied and committed_on_release:
			...
			print(
				(
					"FAIL menu-scale drag: previewed_not_applied=%s committed=%s idx=%d slider=%d"
					% [
						previewed_not_applied,
						committed_on_release,
						int(sm_drag.get("menu_scale_index")),
	...
	if list != null and list.get_child_count() > 0:
		...
		print("FAIL keybinding list empty or missing")
		failed += 1
	...
	if list != null and OS.is_debug_build():
		...
		if has_force_levelup_row and has_growth_boost_row:
			print("OK  debug-only keybinding rows present in debug build")
			passed += 1
		else:
			print(
				(
					"FAIL debug rows missing: force_levelup=%s growth_boost=%s"
		...
		if has_hotseat_row:
			print("OK  V026-01c hotseat debug keybinding row present")
			passed += 1
		else:
			print("FAIL V026-01c hotseat debug keybinding row missing")
			failed += 1

		var debug_rows_read_only: bool = (
			screen._keybind_rows.has("debug_toggle_force_levelup")
		...
		if debug_rows_read_only:
			print("OK  debug keybinding rows stay read-only")
			passed += 1
		else:
			print("FAIL debug keybinding rows exposed edit controls")
			failed += 1
	...
	if sm_bind != null and list != null:
		...
		ev_f.pressed = true
		screen._input(ev_f)
		var derived_conflict: bool = (
			screen._keybind_conflicts.has("confirm")
		...
		confirm_rebind.pressed.emit()
		var capture_started: bool = (
			screen._capturing_action == "confirm" and confirm_rebind.text == "Press key..."
		)
		...
		if not screen._modal_focus_repeat_enabled():
			...
			print("FAIL modal focus repeat active during keybind capture")
			failed += 1
		var ev_y := InputEventKey.new()
		ev_y.keycode = KEY_Y
		ev_y.pressed = true
		screen._input(ev_y)
		var staged_not_live: bool = (
			screen._pending_keybindings.has("confirm")
		...
		apply_btn = screen._btn_apply_keybindings
		var reverted: bool = (
			screen._pending_keybindings.is_empty()
			and not _has_key("confirm", KEY_Y)
			and apply_btn.disabled
		)
		if capture_started and staged_not_live and reverted:
			...
			print(
				(
					"FAIL staged capture: started=%s staged=%s reverted=%s"
					% [capture_started, staged_not_live, reverted]
				)
			)
			failed += 1
		...
		var conflict_rows_red: bool = (
			screen._keybind_conflicts.has("confirm")
			and screen._keybind_conflicts.has("cancel")
			and (
				(screen._keybind_rows["confirm"]["row"] as HBoxContainer).modulate
				== screen._KEYBIND_CONFLICT_COLOR
			)
			and (cancel_info["row"] as HBoxContainer).modulate == screen._KEYBIND_CONFLICT_COLOR
		)
		var conflict_blocks_apply: bool = (
			apply_btn.disabled
		...
		if conflict_rows_red and conflict_blocks_apply:
			...
			print(
				"FAIL conflict state: rows=%s apply=%s" % [conflict_rows_red, conflict_blocks_apply]
			)
			failed += 1
		...
		if clear_resolved and applied:
			...
			print("FAIL clear/apply: resolved=%s applied=%s" % [clear_resolved, applied])
			failed += 1
		...
		var escape_aborted: bool = (
			screen._capturing_action == "" and screen._pending_keybindings.is_empty()
		)
		if escape_aborted:
			print("OK  Esc aborts capture without staging a binding")
			passed += 1
		else:
			print("FAIL Esc did not abort capture")
			failed += 1
		...
		if reset_ok:
			print("OK  Reset Controls restores default keybindings")
			passed += 1
		else:
			print("FAIL Reset Controls did not restore defaults")
			failed += 1

		var confirm_pad_rebind: Button = screen._keybind_rows["confirm"]["pad_rebind"]
		confirm_pad_rebind.pressed.emit()
		var pad_capture_started: bool = (
			screen._capturing_action == "confirm"
		...
		if pad_applied:
			...
			print(
				(
					"FAIL pad capture/apply: started=%s conflict=%s applied=%s"
					% [pad_capture_started, pad_conflict, pad_applied]
				)
			)
			failed += 1
	...
	if margin != null and margin.get_theme_constant("margin_right") > 0:
		print("OK  V026-01b right margin present between rows and scrollbar")
		passed += 1
	else:
		print("FAIL V026-01b scrollbar right margin missing")
		failed += 1
	...
	for factor in [2.0, 0.5]:
		...
		var center_off: float = absf(panel.global_position.x + panel.size.x * 0.5 - vp_w * 0.5)
		...
		if panel.size.x + 0.5 < minf(min_w, vp_w):
			...
			print(
				(
					"FAIL V026-01a panel narrower than content at %sx: %.1f < %.1f"
	...
	if sm_res != null:
		...
		screen.open()
		var custom_ok: bool = (
			opt_res.item_count == preset_count + 1
		...
		if custom_ok and resync_ok and preset_ok:
			...
			print(
				(
					"FAIL V027-04b dropdown: custom=%s resync=%s preset=%s items=%d sel=%d"
	...
	if sm_q6 != null:
		...
		var native_label_ok: bool = (
			String(lbl_q6.text).begins_with("native ") or String(lbl_q6.text) == ""
		)
		var disabled_ok: bool = opt_q6.disabled and native_label_ok
		sm_q6.set("window_mode", "windowed")
		screen.open()
		var reenabled_ok: bool = (
			not opt_q6.disabled
			and String(sm_q6.get("resolution")) == "2560x1440"
			and opt_q6.selected == 3
		)  # the preserved 1440p request
		...
		if disabled_ok and reenabled_ok:
			...
			print(
				(
					"FAIL V027-05c gray-out: disabled=%s label='%s' reenabled=%s sel=%d"
	...
	if sm_live_size != null and sm_live_size.has_signal("display_size_changed"):
		...
		await process_frame
		var display_signal_refresh_ok: bool = (
			String(lbl_live.text) == "client 1800x1013"
		...
		if display_signal_refresh_ok:
			...
			print(
				(
					"FAIL V030D-DSP-01 display signal refresh: before='%s' after='%s' disabled=%s"
	...
	var max_label: String = screen._applied_size_text(
		true, true, Vector2i(2368, 1310), Vector2i.ZERO, {}
	)
	var custom_label: String = screen._applied_size_text(
		true,
		false,
		Vector2i.ZERO,
		Vector2i.ZERO,
		{"kind": "custom", "applied": Vector2i(1800, 1013)}
	)
	var native_label: String = screen._applied_size_text(
		false, false, Vector2i.ZERO, Vector2i(3840, 2160), {}
	)
	if (
		max_label == "Maximized (2368x1310)"
		and custom_label == "client 1800x1013"
		...
		print(
			(
				"FAIL V030-DSP-01 labels: max=%s custom=%s native=%s"
	...
	if imm != null and sm_mode != null:
		...
		var roundtrip_ok: bool = (
			opt_mode != null and opt_mode.selected == mode_values.find("gamepad")
		)
		...
		if items_ok and disable_ok and roundtrip_ok:
			...
			print(
				(
					"FAIL input-mode selector: items=%s disable=%s roundtrip=%s"
	...
	if root.get_node_or_null("SettingsManager") != null:
		screen.open()
		var back_btn: Button = screen.get_node_or_null("Panel/ScrollContainer/Margin/VBox/BtnBack")
		...
		if grabbed_back and released and hidden_noop:
			...
			print(
				(
					"FAIL focus-grab: grabbed=%s released=%s hidden_noop=%s"
