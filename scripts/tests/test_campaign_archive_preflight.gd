...
func _init() -> void:
	...
	if valid.valid and valid.package_id == ROOT:
		print("OK  valid in-memory fixture preflights without extraction"); passed += 1
	else:
		print("FAIL valid fixture: %s" % [valid.errors]); failed += 1
	...
	if zip_result.valid and zip_result.package_id == ROOT:
		print("OK  actual ZIP format and central directory preflight successfully"); passed += 1
	else:
	...
	if not unsafe_result.valid and _has(unsafe_result.errors, "Unsafe archive path"):
		print("OK  traversal, absolute/drive, and backslash paths are rejected"); passed += 1
	else:
		print("FAIL unsafe paths: %s" % [unsafe_result.errors]); failed += 1
	...
	if not collision_result.valid and _has(collision_result.errors, "Duplicate normalized") \
			and _has(collision_result.errors, "Case-fold path collision"):
		print("OK  duplicate and case-fold collisions are rejected"); passed += 1
	else:
		print("FAIL collisions: %s" % [collision_result.errors]); failed += 1
	...
	if not types_result.valid and _has(types_result.errors, "forbidden symlink") \
			and _has(types_result.errors, "forbidden special file"):
		print("OK  symlinks and special files are rejected from ZIP metadata"); passed += 1
	else:
		print("FAIL hostile types: %s" % [types_result.errors]); failed += 1

	var tiny_limits = Preflight.Limits.new(2, 4, 4, 8, 8)
	var size_result = Preflight.inspect_entries(entries, payloads, tiny_limits)
	if not size_result.valid and _has(size_result.errors, "size limits") \
			and _has(size_result.errors, "entries; limit"):
		print("OK  caller-owned entry and byte budgets are enforced"); passed += 1
	else:
		print("FAIL size limits: %s" % [size_result.errors]); failed += 1

	var save_payloads := payloads.duplicate(true)
	var save_path := "%s/data/save.json" % ROOT
	save_payloads[save_path] = _json_bytes({
		"format_version": 1, "save_label": "slot", "map_runtime": {}, "suspend": {},
	})
	var catalogue: Dictionary = JSON.parse_string(save_payloads["%s/data/catalogue.json" % ROOT].get_string_from_utf8())
	...
	if not save_result.valid and _has(save_result.errors, "Save-shaped JSON"):
		print("OK  indexed save-shaped payloads are never accepted as pack content"); passed += 1
	else:
		print("FAIL save-shaped payload: %s" % [save_result.errors]); failed += 1

	var bad_payloads := payloads.duplicate(true)
	bad_payloads["%s/manifest.json" % ROOT] = _json_bytes({
		"id": "different", "version": "1", "builder_content_version": "0.4",
		"format_version": 99,
	})
	...
	if not bad_result.valid and _has(bad_result.errors, "unsupported format_version") \
			and _has(bad_result.errors, "Invalid JSON") \
			and _has(bad_result.errors, "Unindexed file"):
		print("OK  incompatible, malformed, and unindexed content fail together"); passed += 1
	else:
		print("FAIL malformed package: %s" % [bad_result.errors]); failed += 1
...
func _entry(path: String, size: int, file_type := "file") -> Dictionary:
	return {
		"path": path, "compressed_size": size, "uncompressed_size": size,
		"file_type": file_type, "is_directory": file_type == "directory",
	}
