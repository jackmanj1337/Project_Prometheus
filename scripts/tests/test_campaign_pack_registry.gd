...
func _init() -> void:
	...
	if (
		...
		and summaries[0]["campaigns"].any(_is_alpha_campaign)
		and summaries[0]["campaigns"].any(_is_single_map_campaign)
	):
		print("OK  valid installed packs produce deterministic campaign summaries")
		passed += 1
	else:
		print("FAIL discovery summaries: %s" % [summaries])
		failed += 1

	if (
		registry.errors().size() >= 2
		...
		print("OK  malformed and path-mismatched candidates are excluded with diagnostics")
		passed += 1
	else:
		print("FAIL discovery diagnostics: %s" % [registry.errors()])
		failed += 1

	var found := registry.find("zeta", "2.0")
	found["campaigns"].clear()
	if (
		registry.find("zeta", "2.0")["campaigns"].size() == 2
		and registry.find("missing", "1.0").is_empty()
	):
		print("OK  summary reads are deep-copy cached and exact-identity keyed")
		passed += 1
	else:
		print("FAIL summary cache isolation")
		failed += 1
	...
	if refreshed.size() == 1 and refreshed[0]["package_id"] == "zeta":
		print("OK  refresh drops removed packs instead of retaining stale summaries")
		passed += 1
	else:
		print("FAIL stale summary refresh: %s" % [refreshed])
		failed += 1
...
func _write_pack(
	root: String, id: String, version: String, campaign_id: String, label: String
) -> void:
	var files := {
		"manifest.json":
		{
...
func _is_alpha_campaign(row: Dictionary) -> bool:
	return row["campaign_id"] == "a_campaign" and row["label"] == "Alpha"


func _is_single_map_campaign(row: Dictionary) -> bool:
