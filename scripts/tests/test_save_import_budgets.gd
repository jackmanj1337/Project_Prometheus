...
func _init() -> void:
	...
	for row in rows:
		print("MEASURE save budget %s: %d JSON bytes, parse %d usec, observed memory delta %d bytes" % [
			row["id"], row["json_bytes"], row["parse_usec"],
			row["observed_memory_delta_bytes"]])
		if row["parsed"] and row["json_bytes"] < ImportBudgetConfig.portable_save_warning_bytes() \
