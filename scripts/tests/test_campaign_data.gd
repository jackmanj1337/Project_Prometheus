...
func _init() -> void:
	...
	if shipped != null and shipped.campaign_id == "proving_grounds" \
			and shipped.label != "" and shipped.nodes.size() == 5 \
	...
	if live_errors.is_empty():
		print("OK  shipped campaign map bindings resolve against the map registry"); passed += 1
	else:
		print("FAIL live campaign validation: %s" % [live_errors]); failed += 1
	...
	if shipped != null and shipped.node_ids() == expected_order \
			and shipped.node_ids() == shipped.node_ids() \
			and shipped.start_node_id == "node_01_rout":
		print("OK  node_ids() returns the authored order deterministically"); passed += 1
	else:
		print("FAIL node ordering: %s" % [shipped.node_ids() if shipped != null else "null"]); failed += 1
	...
	var terminal: CampaignNode = shipped.get_node_by_id("node_05_defend") if shipped != null else null
	if walked == expected_order and terminal != null and terminal.is_terminal():
		print("OK  linear traversal from start reaches the terminal node"); passed += 1
	else:
		print("FAIL traversal: walked=%s" % [walked]); failed += 1

	# ---- unknown node id resolves to null, not a silent fallback to node 1 ----
	if shipped != null and shipped.get_node_by_id("no_such_node") == null \
			and not shipped.has_node("no_such_node") \
			and shipped.next_node_ids_of("no_such_node").is_empty():
		print("OK  unknown node id returns null / empty rather than a fallback"); passed += 1
	else:
		print("FAIL unknown node id did not fail closed"); failed += 1
	...
	var map_errors: Array[String] = DataManagerScript.collect_campaign_validation_errors(
		{"dangling": dangling},
		DataManagerScript.collect_map_registry_ids("res://data/maps/map_registry.json"))
	if dangling != null and map_errors.size() == 1 \
			and map_errors[0].contains("unknown map id 'map_does_not_exist'"):
		print("OK  a node bound to an unknown map id fails reference validation"); passed += 1
	else:
	...
	var malformed_cases := [
		...
		["unknown start_node_id", {
			"campaign_id": "bad", "label": "Bad", "start_node_id": "ghost",
			"nodes": [{"node_id": "n1", "map_id": "map_001", "next": []}],
		...
		["required and excluded overlap", {
			...
			"nodes": [{
				"node_id": "n1", "map_id": "map_001", "next": [],
				"required_units": ["lyn"], "excluded_units": ["lyn"],
			}],
	...
	for case_name in null_cases:
		...
		if CampaignData.parse(null_cases[case_name], "fixture", errors) != null or errors.is_empty():
			print("FAIL %s should parse to null with an error" % case_name)
			null_ok = false
	if null_ok:
		print("OK  unrepresentable campaign documents parse to null with errors"); passed += 1
	else:
		failed += 1

	# ---- a branching graph is representable with no reshape ----
	var branching := _parse_ok({
		"campaign_id": "branching", "label": "Branching",
		"nodes": [
	...
	if branching != null and branching.next_node_ids_of("n1") == ["left", "right"]:
		print("OK  a branching graph parses without a schema reshape"); passed += 1
	else:
		print("FAIL branching graph did not parse"); failed += 1
