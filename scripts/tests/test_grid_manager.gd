...
func _init() -> void:
	...
	if has_origin and no_wall and cant_reach_far:
		...
		print(
			(
				"FAIL movement range: origin=%s no_wall=%s no_far=%s"
				% [has_origin, no_wall, cant_reach_far]
			)
		)
		failed += 1
...
"""
	...
	if has_outside and no_inside:
		...
		print(
			(
				"FAIL staff range: outside=%s no_inside=%s ring=%s"
				% [has_outside, no_inside, heal_ring]
			)
		)
		failed += 1
	...
	if (
		...
		print(
			(
				"FAIL get_terrain_bonuses: forest=%s mountain=%s wall=%s plain=%s"
	...
	if mt_ok:
		print("OK  V021-11 movement_type_of resolves by precedence, ignores non-move tags")
		passed += 1
	else:
		print("FAIL V021-11 movement_type_of")
		failed += 1

	# Flying ignores ground terrain (flat 1) but walls still block.
	var flier := MockTypedUnit.new(Vector2i(0, 0), ["flying"])
	if (
		grid.get_move_cost(Vector2i(2, 2), flier) == 1
		and grid.get_move_cost(Vector2i(3, 3), flier) == 1
		and grid.get_move_cost(Vector2i(4, 4), flier) == GridManager.IMPASSABLE_MOVE_COST
	):
		print("OK  V021-11 flier pays 1 on forest/mountain, blocked by wall")
		passed += 1
	else:
		print(
			(
				"FAIL V021-11 flier costs: forest=%d mountain=%d wall=%d"
				% [
					grid.get_move_cost(Vector2i(2, 2), flier),
					grid.get_move_cost(Vector2i(3, 3), flier),
					grid.get_move_cost(Vector2i(4, 4), flier)
				]
			)
		)
		failed += 1
	...
	if (
		grid.get_move_cost(Vector2i(0, 5), armoured) == 3
		and grid.get_move_cost(Vector2i(0, 5), light) == 1
		and grid.get_move_cost(Vector2i(0, 5), foot) == 2
	):
		print("OK  V021-11 desert costs by resolved type (armoured 3, light 1, infantry 2)")
		passed += 1
	else:
		print(
			(
				"FAIL V021-11 desert costs: armoured=%d light=%d infantry=%d"
				% [
					grid.get_move_cost(Vector2i(0, 5), armoured),
					grid.get_move_cost(Vector2i(0, 5), light),
					grid.get_move_cost(Vector2i(0, 5), foot)
				]
			)
		)
		failed += 1
	...
	if (
		int(GridManager.get_move_costs_for_groups("mountain").get("flying", -1)) == 1
		and (
			int(GridManager.get_move_costs_for_groups("wall").get("flying", -1))
			== GridManager.IMPASSABLE_MOVE_COST
		)
	):
		print("OK  V021-11 get_move_costs_for_groups includes flying (1 / wall impassable)")
		passed += 1
	else:
		print("FAIL V021-11 flying group column")
		failed += 1
	...
	if threat.size() > 4 and start_adjacent == 4:
		print(
			(
				"OK  [TUR-1] get_unit_threat_tiles = reach ∪ attack-from-reach (%d tiles)"
				% threat.size()
			)
		)
		passed += 1
	else:
		print("FAIL [TUR-1] threat tiles: size=%d adj=%d" % [threat.size(), start_adjacent])
		failed += 1
	...
	dead.data.hp = 0
	if (
		tgrid.get_unit_threat_tiles(null).is_empty()
		and tgrid.get_unit_threat_tiles(dead).is_empty()
	):
		print("OK  [TUR-1] null/dead unit threatens no tiles")
		passed += 1
	else:
		print("FAIL [TUR-1] null/dead unit produced threat tiles")
		failed += 1
	...
	if (
		staff != null
		and staff.is_healing_staff()
		and tgrid.get_unit_threat_tiles(healer).is_empty()
	):
		print("OK  [TUR-1] a healer contributes no threat tiles")
		passed += 1
	else:
		print(
			(
				"FAIL [TUR-1] healer threat: staff=%s tiles=%d"
				% [staff, tgrid.get_unit_threat_tiles(healer).size()]
			)
		)
		failed += 1
	...
	if p_move < p_faction and p_faction < p_watch:
		print("OK  [MRD-1] built-in overlay precedence: move < faction < watch")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-1] precedence order: move=%d faction=%d watch=%d"
				% [p_move, p_faction, p_watch]
			)
		)
		failed += 1
	...
	var layer_ids := [
		...
		"fixture_healing_zone",
		GridManager.OVERLAY_LAYER_MOVE,
		GridManager.OVERLAY_LAYER_FACTION_THREAT,
	]
	layer_ids.sort_custom(
		func(a, b):
			return GridManager.overlay_layer_precedence(a) < GridManager.overlay_layer_precedence(b)
	)
	var order_ok: bool = (
		layer_ids[0] == GridManager.OVERLAY_LAYER_MOVE
	...
	if p_fix == 25 and order_ok and p_unknown > p_watch:
		print("OK  [MRD-1] a newly-registered layer slots by precedence (no repaint edit)")
		passed += 1
	else:
		print("FAIL [MRD-1] fixture layer: p=%d order=%s unknown=%d" % [p_fix, order_ok, p_unknown])
		failed += 1
	...
	(
		...
		. repaint_overlays(
			{
				GridManager.OVERLAY_LAYER_FACTION_THREAT:
				{"tiles": faction_tiles, "source": GridManager.OVERLAY_DARK_RED},
				GridManager.OVERLAY_LAYER_WATCH_THREAT:
				{"tiles": watch_tiles, "source": GridManager.OVERLAY_DARKER_RED},
			}
		)
	)
	...
	if (
		shared_src == GridManager.OVERLAY_DARKER_RED
		and faction_only_src == GridManager.OVERLAY_DARK_RED
	):
		print(
			"OK  [MRD-1] repaint_overlays: watch (src4) wins the shared cell, faction (src3) elsewhere"
		)
		passed += 1
	else:
		print("FAIL [MRD-1] paint order: shared=%d faction=%d" % [shared_src, faction_only_src])
		failed += 1
	ov.free()
	...
	if (
		border_shared == GridManager.OVERLAY_BLUE_ON_DARKER_RED
		and border_move == GridManager.OVERLAY_BLUE
		and border_watch == GridManager.OVERLAY_DARKER_RED
		and border_top_empty
	):
		print("OK  [MRD-7] border-through mode paints a combined source on shared cells")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-7] border mode: shared=%d move=%d watch=%d top_empty=%s"
				% [border_shared, border_move, border_watch, border_top_empty]
			)
		)
		failed += 1
	...
	if (
		stacked_base_shared == GridManager.OVERLAY_DARKER_RED
		...
		print("OK  [MRD-7] stacked mode separates threat and range across overlay layers")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-7] stacked mode: base_shared=%d top_shared=%d base_watch=%d top_move=%d"
				% [stacked_base_shared, stacked_top_shared, stacked_base_watch, stacked_top_move]
			)
		)
		failed += 1
	...
	var masks_ok: bool = (
		...
		and (
			...
			== (
				...
				| GridManager.PERIMETER_EDGE_BOTTOM
		...
		and (
			concave_corner_mask
			== (GridManager.PERIMETER_EDGE_TOP | GridManager.PERIMETER_EDGE_LEFT)
		)
	)
	if masks_ok:
		print("OK  [MRD-7] perimeter masks cover isolated, adjacent, and concave threat tiles")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-7] perimeter masks: iso=%d line=(%d,%d) concave=%d"
				% [iso_mask, line_left_mask, line_right_mask, concave_corner_mask]
			)
		)
		failed += 1
	...
	while seg_scan + 1 < line_segments.size():
		...
		if (
			(seg_a == Vector2(64, 0) and seg_b == Vector2(64, 64))
			or (seg_a == Vector2(64, 64) and seg_b == Vector2(64, 0))
		):
			inner_edge_absent = false
		seg_scan += 2
	if seg_counts_ok and inner_edge_absent:
		print("OK  [MRD-7] dual-outline segments stroke perimeters only, never shared inner edges")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-7] outline segments: iso=%d line=%d inner_absent=%s"
				% [iso_segments.size(), line_segments.size(), inner_edge_absent]
			)
		)
		failed += 1
	...
	var perimeter_expected_mask: int = (
		GridManager.PERIMETER_EDGE_TOP
		| GridManager.PERIMETER_EDGE_RIGHT
		| GridManager.PERIMETER_EDGE_LEFT
	)
	...
	if (
		...
		print("OK  [MRD-7] stacked-perimeter mode paints threat edge masks under range fill")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-7] stacked-perimeter: base=%d want=%d top_shared=%d top_move=%d"
				% [
					perimeter_base_shared,
					perimeter_expected_source,
					perimeter_top_shared,
					perimeter_top_move
				]
			)
		)
	...
	shared_grid.set_shared_cell_mode(GridManager.SHARED_CELL_STACKED)
	var dual_cleared: bool = (
		outline == null
		or (outline._watch_segments.is_empty() and outline._danger_segments.is_empty())
	)
	if dual_ok and dual_cleared:
		print("OK  [MRD-7] dual_outline splits watch/danger perimeters above the stacked fill")
		passed += 1
	else:
		print(
			(
				"FAIL [MRD-7] dual_outline: outline=%s watch=%d danger=%d base=%d top=%d cleared=%s"
				% [
					outline,
					outline._watch_segments.size() if outline != null else -1,
