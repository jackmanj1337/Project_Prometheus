...
func _init() -> void:
	...
	if _is_legal(
		"a plan on distinct start tiles is legal", {"lyn": TILES[0], "kent": TILES[1]}, party, null
	):
		passed += 1
	else:
		failed += 1

	if _rejects("an empty plan deploys nobody", {}, party, null, "deploys no units"):
		passed += 1
	else:
		failed += 1

	if _rejects(
		"a unit outside the party cannot deploy",
		{"eliwood": TILES[0]},
		party,
		null,
		"not in the party"
	...
	if _rejects(
		"a tile that is not a player start tile is rejected",
		{"lyn": Vector2i(9, 9)},
		party,
		null,
		"not a player start tile"
	):
		passed += 1
	else:
		failed += 1
	...
	if _rejects(
		"a plan larger than the start-tile count is rejected",
		{
			"lyn": TILES[0],
			"kent": TILES[1],
			"sain": TILES[2],
	...
	if _rejects(
		"a fallen unit cannot be deployed", {"kent": TILES[0]}, fallen_party, null, "has fallen"
	):
		passed += 1
	else:
		failed += 1
	...
	if _is_legal(
		"a plan at the deployment cap is legal", {"lyn": TILES[0], "kent": TILES[1]}, party, capped
	):
		passed += 1
	else:
		failed += 1
	...
	if _rejects(
		"a plan over the deployment cap is rejected",
		{"lyn": TILES[0], "kent": TILES[1], "sain": TILES[2]},
		party,
		capped,
		"caps deployment at 2"
	):
		passed += 1
	else:
		failed += 1
	...
	if _is_legal(
		"deployment_cap -1 is uncapped, not zero",
	...
	if _rejects(
		"an excluded unit cannot be deployed",
		{"lyn": TILES[0], "kent": TILES[1]},
		party,
		_node("n1", [], ["kent"], -1),
		"excludes unit 'kent'"
	):
	...
	if _rejects(
		"a required unit cannot be benched",
		{"kent": TILES[0]},
		party,
		_node("n1", ["lyn"], [], -1),
		"requires unit 'lyn' to deploy"
	):
		passed += 1
	else:
		failed += 1
	...
	if _is_legal(
		"a deployed required unit satisfies the requirement",
		{"lyn": TILES[0]},
		party,
		_node("n1", ["lyn"], [], -1)
	):
	...
	if _is_legal(
		"a fallen required unit is excused, not a launch block",
		{"lyn": TILES[0]},
		[_unit("lyn"), _fallen("kent")] as Array[UnitData],
		_node("n1", ["kent"], [], -1)
	):
		passed += 1
	else:
	...
	if _rejects(
		"a required unit missing from the party is an authoring error",
		{"lyn": TILES[0]},
		party,
		_node("n1", ["eliwood"], [], -1),
		"which is not in the party"
	):
		passed += 1
	else:
		failed += 1
	...
	if _is_legal(
		"a null node skips the node constraints entirely",
		{"lyn": TILES[0], "kent": TILES[1], "sain": TILES[2], "florina": TILES[3]},
		party,
		null
	):
...
static func _node(
	node_id: String, required: Array[String], excluded: Array[String], cap: int
) -> CampaignNode:
...
static func _is_legal(
	case_name: String, plan: Dictionary, party: Array[UnitData], node: CampaignNode
) -> bool:
...
static func _rejects(
	case_name: String, plan: Dictionary, party: Array[UnitData], node: CampaignNode, needle: String
) -> bool:
	var errors: Array[String] = DeploymentPlanS.validate(plan, party, node, TILES)
	for err in errors:
