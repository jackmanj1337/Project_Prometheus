...
func _test_mid_map_slot_write_and_load(manager: Node) -> void:
	...
	_check(
		...
		"mid-map save uses the unified slot and continue pointer",
		str(last_played)
	)
	...
	_check(
		(
			...
			and (
				loaded.ledger[0]["entry"]["map_runtime"]["rng"]
...
func _test_mid_map_slot_rejects_bad_documents(manager: Node) -> void:
	...
	_check(
		manager.load_slot(SaveManagerScript.MID_MAP_SLOT) == null,
...
func _test_slot_write_and_load(manager: Node) -> void:
	var write_ok: bool = manager.save_slot(
		"autosave", _make_campaign_save(), "auto", "campaign_progress"
	)
	var last_played: Dictionary = manager.get_last_played()
	_check(
		(
		...
		"save_slot writes the slot file and points continue at it",
		str(last_played)
	)
	...
	_check(
		manager.load_slot("never_written") == null,
...
func _test_slot_ids_are_allow_listed(manager: Node) -> void:
	_check(
		(
			SaveManagerScript.is_valid_slot_id("autosave")
	...
	_check(
		(
			not SaveManagerScript.is_valid_slot_id("")
	...
	_check(
		(
			not manager.save_slot("../evil", _make_campaign_save())
...
func _test_continue_routes_to_the_newest_document(manager: Node) -> void:
	...
	_check(
		...
		"continue routes to the campaign slot when it is newest",
		str(target)
	)
	...
	_check(
		...
		"continue routes to the mid-map slot when it is newest",
		str(target)
	)
	...
	_check(
		...
		"continue falls back to the surviving save when the newest is gone",
		str(target)
	)
...
func _test_completed_slots_are_records_not_continue_targets(manager: Node) -> void:
	...
	var target: Dictionary = manager.get_continue_target()
	_check(
		String(target.get("slot_id", "")) == "autosave",
		"Continue skips a newer completed record",
		str(target)
	)
	manager.delete_slot("autosave")
	_check(
		(
			manager.get_continue_target().is_empty()
			and not manager.has_continue_save()
...
func _test_slot_listing(manager: Node) -> void:
	...
	_check(
		rows.size() == 3 and ids.has("autosave") and ids.has("manual_01") and ids.has("completed"),
		"list_slots returns a row per written slot",
		str(ids)
	)
	_check(
		(
			String(rows[0].get("slot_id", "")) == "manual_01"
	...
	_check(survivors.size() == 2, "a slot whose file has vanished is not listed", str(survivors))
...
func _test_portable_save_transfer_and_integrity(manager: Node) -> void:
	...
	_check(
		(
			exported["ok"]
			and String(parsed.get("integrity", {}).get("payload_hash", "")) != ""
	...
	_check(
		(
			replacement["ok"]
			and FileAccess.get_file_as_string(portable) == first_export
	...
	_check(
		clean["ok"] and clean["warnings"].is_empty(),
		"an untouched portable save verifies without warnings",
		str(clean)
	)
	...
	_check(
		(
			large_warning["ok"]
			and large_warning["warnings"].size() == 1
	...
	_check(
		accepted_large["ok"] and manager.has_slot("large_import"),
	...
	_check(
		(
			not warned["ok"]
			and warned.get("requires_acknowledgement", false)
			and warned["warnings"].size() == 2
			and not manager.has_slot("imported_01")
		),
		"tampered protected data requires acknowledgement before import",
		str(warned)
	)
	...
	_check(
		(
			accepted["ok"]
			and manager.has_slot("imported_01")
			and manager.load_slot("imported_01").campaign["node_id"] == "tampered_node"
		),
		"acknowledged tampering warns-and-continues into a normal slot",
		str(accepted)
	)
	...
	_check(
		not sniffed["ok"] and sniffed["artifact_kind"] == "campaign_pack",
	...
	_check(
		(
			not oversized_result["ok"]
			and oversized_result["errors"].any(func(error): return "size limit" in String(error))
		),
...
func _test_campaign_preference_order(manager: Node) -> void:
	manager.record_campaign_imported(
		{"campaign_id": "imported_campaign", "package_id": "pack", "package_version": "1.0"}
	)
...
func _test_slot_delete(manager: Node) -> void:
	...
	_check(
		not manager.has_continue_save() and manager.get_continue_target().is_empty(),
...
func _write_text(path: String, text: String) -> void:
	...
	if file == null:
		push_error(
			(
				"test_save_manager: failed to write test file: %s"
