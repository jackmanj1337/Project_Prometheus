...
func _init() -> void:
	...
	if (
		loaded_ok
		and bool(gs.get("configured"))
		and (
			String(gs.get("configured_map_path"))
			== "res://data/maps/map_001_rout/map_001_data.tres"
		)
	):
		...
		print(
			(
				"FAIL Continue stage: ok=%s configured=%s path=%s"
	...
	if failed_load_ok and not bool(gs.get("configured")) and _consume_error_dialog(menu):
		...
		print(
			(
				"FAIL Continue load failure: failed_ok=%s configured=%s"
	...
	if (
		slot_ok
		and String(gs.get("resumed_campaign_id")) == "proving_grounds"
		...
		print(
			(
				"FAIL campaign slot resume: ok=%s campaign=%s launched=%s"
	...
	if complete_ok and not bool(cm.get("launched")) and _consume_error_dialog(menu):
		...
		print("FAIL completed campaign: refused=%s launched=%s" % [complete_ok, cm.get("launched")])
		failed += 1
	...
	if (
		autosave_text.contains("[Autosave]")
		and not manual_text.contains("[Autosave]")
	...
	var export_ok: bool = (
		FileAccess.file_exists(portable_path)
		and "Exported save" in picker.get_node("TransferResult").dialog_text
	)
	picker._on_import_file_selected(portable_path)
	var import_ok: bool = (
		save_manager.has_slot("imported_01") and picker.get_slot_ids().has("imported_01")
	)
	...
	if (
		completed_btn.disabled
		and completed_btn.text.contains("[Completed]")
		...
		print(
			(
				"FAIL completion row: disabled=%s text=%s"
	...
	if String(gs.get("resumed_campaign_id")) == "proving_grounds" and bool(cm.get("launched")):
		...
		print(
			(
				"FAIL row activate: campaign=%s launched=%s"
	...
	_row_load_button(picker, "manual_01").pressed.emit()
	if (
		String(gs.get("resumed_campaign_id")) == ""
		and not bool(cm.get("launched"))
		...
		print(
			(
				"FAIL corrupt slot: campaign=%s launched=%s"
				% [gs.get("resumed_campaign_id"), cm.get("launched")]
			)
		)
		failed += 1
	...
	var pointed_at_manual: bool = (
		String(save_manager.get_continue_target().get("slot_id", "")) == "manual_01"
	)
	...
	if (
		pointed_at_manual
		and picker.get_slot_ids() == only_autosave
	...
	if not continue_btn.disabled and not load_btn.disabled:
		...
		print(
			(
				"FAIL after delete: continue_disabled=%s load_disabled=%s"
	...
	if continue_btn.disabled and load_btn.disabled and picker.get_slot_ids().is_empty():
		...
		print(
			(
				"FAIL last delete: continue_disabled=%s load_disabled=%s rows=%s"
...
func _write_text(path: String, text: String) -> void:
	...
	if file == null:
		push_error(
			(
				"test_main_menu: failed to write test file: %s"
