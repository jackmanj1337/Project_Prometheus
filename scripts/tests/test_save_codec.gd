...
func _init() -> void:
	...
	if weapon_after != null and weapon_after.entry_type == "weapon" \
			...
			and weapon_after.accuracy == 5 and weapon_after.damage == 2 \
			and weapon_after.crit == 1 and weapon_after.dodge == 3:
	...
	if item_after != null and item_after.is_item() and item_after.item_id == "vulnerary" \
	...
	unit.active_modifiers = [{"stat": "strength", "delta": 2, "source": "test",
	...
	var json_safe := unit_dict["tile_position"] is Dictionary \
	...
	if json_safe:
		...
		print("FAIL unit snapshot shape: tile=%s inv0=%s" % [
	...
	if restored.tile_position == Vector2i(7, 9) and restored.class_id == "cavalier" \
			and restored.hp == 18 and restored.max_hp == 24 \
			and restored.strength == 9 and restored.magic == 2 \
			and restored.defense == 7 and restored.resistance == 4 \
			and restored.skill == 8 and restored.speed == 10 \
			and restored.luck == 6 and restored.exp == 55 \
			and restored.level == 5 and restored.internal_level == 8 \
			...
			and restored.shift_gauge == 12 and restored.is_shifted:
		print("OK  save_codec_unit_roundtrip: unit snapshot survives JSON")
		passed += 1
	else:
		print("FAIL unit roundtrip: tile=%s class=%s hp=%d inv=%s skills=%s mods=%s" % [
			restored.tile_position, restored.class_id, restored.hp,
			restored.inventory, restored.skills, restored.active_modifiers])
		failed += 1
	...
	bad["inventory"] = [
		{"entry_type": "weapon", "weapon_id": "missing_sword", "uses_remaining": 1,
			"forged_mods": {}, "accuracy": 0, "damage": 0, "crit": 0, "dodge": 0},
		{"entry_type": "item", "item_id": "missing_vulnerary", "uses_remaining": 1,
			"forged_mods": {}, "accuracy": 0, "damage": 0, "crit": 0, "dodge": 0},
	]
