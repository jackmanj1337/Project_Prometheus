...
func _make_ctx(
	atk: Node, def: Node, w_atk: WeaponData, w_def: WeaponData, damage: int = 0, sim_hp: int = 17
) -> Dictionary:
	return {
		"attacker": atk,
		"defender": def,
		"attacker_weapon": w_atk,
		"defender_weapon": w_def,
...
func _init() -> void:
	...
	if ctx["flags"].get("vantage", false) and vantage_fired:
		...
		print("FAIL vantage: flag not set / not reported fired when unit is defender")
		failed += 1
	...
	if not ctx2["flags"].has("vantage") and not vantage_fired2:
		...
		print("FAIL vantage: flag set / reported fired when unit is attacker")
		failed += 1
	...
	if ctx3["atk_mod"]["crit"] == 50:
		print("OK  wrath +50 crit when HP ≤ 50%")
		passed += 1
	else:
		print("FAIL wrath: expected crit 50, got %d" % ctx3["atk_mod"]["crit"])
		failed += 1
	...
	if ctx4["atk_mod"]["crit"] == 0 and not wrath_fired_full:
		...
		print("FAIL wrath: bonus applied / reported fired at full HP")
		failed += 1
	...
	if ctx5["damage"] == 9 and miracle_fired:
		...
		print(
			"FAIL miracle: expected damage 9 + fired, got %d / %s" % [ctx5["damage"], miracle_fired]
		)
		failed += 1
	...
	if ctx6["damage"] == 5 and not miracle_fired_nonfatal:
		...
		print("FAIL miracle: modified non-fatal damage / reported fired")
		failed += 1
	...
	if (
		resolve_stats.has("strength")
		and resolve_stats.has("magic")
		and resolve_stats.has("skill")
		and resolve_stats.has("speed")
	):
		print("OK  resolve adds STR/MAG/SKL/SPD modifiers when HP ≤ 50%")
		passed += 1
	else:
		print("FAIL resolve: expected 4 stat modifiers, got %s" % str(resolve_stats.keys()))
		failed += 1
	...
	if ctx8["atk_mod"]["damage"] > 0:
		print("OK  lancefaire adds damage bonus with lance")
		passed += 1
	else:
		print("FAIL lancefaire: no bonus with lance")
		failed += 1
	...
	if ctx9["atk_mod"]["damage"] == 0 and not faire_fired_wrong:
		...
		print("FAIL lancefaire: bonus applied / reported fired with wrong weapon type")
		failed += 1
	...
	if ctx10["atk_mod"]["accuracy"] > 0:
		print("OK  lancebreaker +hit vs lance opponent")
		passed += 1
	else:
		print("FAIL lancebreaker: no hit bonus vs lance")
		failed += 1
	...
	if ctx11["atk_mod"]["accuracy"] == 0:
		print("OK  lancebreaker no bonus vs non-lance opponent")
		passed += 1
	else:
		print("FAIL lancebreaker: bonus applied vs non-lance")
		failed += 1
	...
	if sb_fired and sb_unit.get_effective_stat("skill") == sb_data.skill + 2:
		...
		print(
			(
				"FAIL M9a stat_bonus: fired=%s skill=%d base=%d"
				% [sb_fired, sb_unit.get_effective_stat("skill"), sb_data.skill]
			)
		)
		failed += 1
	...
	if (
		...
		and pre_ctx2["def_mod"]["dodge"] == 0
	):
		print("OK  M9a prescience stays inactive on defense")
		passed += 1
	else:
		print(
			(
				"FAIL M9a prescience defended: def_mod=%s fired=%s"
				% [str(pre_ctx2["def_mod"]), pre_fired2]
			)
		)
		failed += 1
	...
	if pat_fired and pat_ctx["def_mod"]["accuracy"] == 10 and pat_ctx["def_mod"]["dodge"] == 10:
		...
		print("FAIL M9a patience def_mod=%s fired=%s" % [str(pat_ctx["def_mod"]), pat_fired])
		failed += 1
	...
	if focus_fired and focus_ctx["atk_mod"]["crit"] == 10:
		...
		print("FAIL M9a focus solo atk_mod=%s fired=%s" % [str(focus_ctx["atk_mod"]), focus_fired])
		failed += 1
	...
	if not focus_fired2 and focus_ctx2["atk_mod"]["crit"] == 0:
		...
		print(
			"FAIL M9a focus ally atk_mod=%s fired=%s" % [str(focus_ctx2["atk_mod"]), focus_fired2]
		)
		failed += 1
	...
	if wexp_mult == 2 and heal_bonus == 5:
		print("OK  M9a helpers expose discipline/healtouch bonuses")
		passed += 1
	else:
		print("FAIL M9a helpers: wexp=%d heal=%d" % [wexp_mult, heal_bonus])
		failed += 1
	...
	if dmg_after_one > 0 and dmg_after_two == dmg_after_one:
		print("OK  #4: max_uses_per_combat blocks the second activation")
		passed += 1
	else:
		print("FAIL #4: faire fired twice (%d → %d)" % [dmg_after_one, dmg_after_two])
		failed += 1
	# reset_combat_uses() scopes the limit to one combat — a fresh combat fires again.
	sh.reset_combat_uses()
	sh.apply_trigger(combo_unit, "on_combat_start", combo_ctx)
	if combo_ctx["atk_mod"]["damage"] > dmg_after_two:
		print("OK  #4: reset_combat_uses re-arms the skill for the next combat")
		passed += 1
	else:
		print("FAIL #4: skill did not re-arm after reset_combat_uses")
		failed += 1
	...
	if distinct_sources.size() == 2:
		print("OK  2.6: skills sharing effect_id keep isolated use counters")
		passed += 1
	else:
		print(
			(
				"FAIL 2.6: shared-effect-id skill counter collision (%d sources)"
				% distinct_sources.size()
			)
		)
		failed += 1
	...
	if vuln_data.hp == 20:
		print("OK  vulnerary heals exactly 10 HP (#14)")
		passed += 1
	else:
		print("FAIL vulnerary: expected 20 HP, got %d" % vuln_data.hp)
		failed += 1
	if vuln_entry.uses_remaining == 2:
		print("OK  vulnerary decrements uses")
		passed += 1
	else:
		print("FAIL vulnerary: uses not decremented")
		failed += 1
	...
	if full_data2.hp == 17:
		print("OK  vulnerary does not overheal")
		passed += 1
	else:
		print("FAIL vulnerary: overhealed")
		failed += 1
	if vuln_entry2.uses_remaining == 2:
		print("OK  vulnerary still consumed when already full")
		passed += 1
	else:
		print("FAIL vulnerary: uses not consumed at full HP")
		failed += 1
	...
	if last_data.inventory.is_empty():
		print("OK  item removed from inventory when uses reach 0")
		passed += 1
	else:
		print("FAIL item: not removed when uses reach 0")
		failed += 1
	...
	if skip_entry.uses_remaining == 3:
		print("OK  empty item_id is a no-op (uses not consumed)")
		passed += 1
	else:
		print("FAIL empty item_id: uses were consumed")
		failed += 1
	...
	if inf_data.hp > 10 and inf_entry.uses_remaining == -1 and inf_data.inventory.size() == 1:
		print("OK  B1: -1 item heals but is never consumed or removed")
		passed += 1
	else:
		print(
			(
				"FAIL B1 item: hp=%d uses=%d inv=%d"
				% [inf_data.hp, inf_entry.uses_remaining, inf_data.inventory.size()]
			)
		)
		failed += 1

	# ── M6.4: promotion-item restrictions by exact class and class group ─────
	var archer_data: UnitData = load("res://data/roster/default/unit_03_archer.tres").duplicate(
		true
	)
	...
	var merc_data: UnitData = load("res://data/roster/default/unit_02_mercenary.tres").duplicate(
		true
	)
	merc_data.level = 20
	...
	if (
		ih.can_apply_item(archer_unit, orion_entry)
		and not ih.can_apply_item(merc_unit, orion_entry)
	):
		print("OK  M6.4: allowed_classes restrict a promotion item to exact classes")
		passed += 1
	else:
		print(
			(
				"FAIL M6.4 allowed_classes: archer=%s merc=%s"
				% [
					ih.can_apply_item(archer_unit, orion_entry),
					ih.can_apply_item(merc_unit, orion_entry)
				]
			)
		)
		failed += 1
	if ih.can_apply_item(mage_unit, ring_entry) and not ih.can_apply_item(merc_unit, ring_entry):
		print("OK  M6.4: allowed_class_groups restrict a promotion item by class group")
		passed += 1
	else:
		print(
			(
				"FAIL M6.4 allowed_class_groups: mage=%s merc=%s"
				% [
					ih.can_apply_item(mage_unit, ring_entry),
					ih.can_apply_item(merc_unit, ring_entry)
				]
			)
		)
		failed += 1
	...
	if (
		ih.can_apply_item(archer_unit, second_entry)
		and not ih.can_apply_item(merc_unit, second_entry)
	):
		print("OK  M7.4: Second Seal usability follows unit.can_use_second_seal()")
		passed += 1
	else:
		print(
			(
				"FAIL M7.4 second_seal: archer=%s merc=%s"
				% [
					ih.can_apply_item(archer_unit, second_entry),
					ih.can_apply_item(merc_unit, second_entry)
				]
			)
		)
		failed += 1
	...
	var tonic_data: UnitData = load("res://data/roster/default/unit_02_mercenary.tres").duplicate(
		true
	)
	...
	for mod in tonic_data.active_modifiers:
		if (
			mod.get("stat", "") == "strength"
			and int(mod.get("delta", 0)) == 4
	...
	if tonic_usable and matched == 1 and tonic_data.inventory.is_empty():
		print("OK  W5c: strength_tonic applies a +4 strength turn modifier and is consumed")
		passed += 1
	else:
		print(
			(
				"FAIL W5c: usable=%s matched=%d inv=%d mods=%s"
				% [tonic_usable, matched, tonic_data.inventory.size(), tonic_data.active_modifiers]
			)
		)
		failed += 1
	...
	var debuff_data: UnitData = load("res://data/roster/default/unit_02_mercenary.tres").duplicate(
		true
	)
	...
	if debuff_usable and debuff_matched == 1 and debuff_data.inventory.is_empty():
		print("OK  V020-14: debuff_tonic applies a -4 strength turn modifier and is consumed")
		passed += 1
	else:
		print(
			(
				"FAIL V020-14: usable=%s matched=%d inv=%d mods=%s"
				% [
					debuff_usable,
					debuff_matched,
					debuff_data.inventory.size(),
					debuff_data.active_modifiers
				]
			)
		)
	...
	var refresh_data: UnitData = load("res://data/roster/default/unit_06_knight.tres").duplicate(
		true
	)
	...
	if hidden_at_18 and hidden_at_19 and usable_at_20:
		print("OK  W3b: promotion-item usability refreshes the instant a unit hits max level")
		passed += 1
	else:
		print(
			(
				"FAIL W3b promotion refresh: 18=%s 19=%s 20=%s"
				% [hidden_at_18, hidden_at_19, usable_at_20]
			)
		)
		failed += 1
	...
	if (
		sh._stub_warned.size() == 2
		and sh._stub_warned.has("armsthrift")
		and sh._stub_warned.has("dash")
	):
		print("OK  stub appliers warn once per skill id (repeats suppressed)")
		passed += 1
	else:
		print("FAIL stub warn dedupe: %s" % sh._stub_warned)
		failed += 1
	...
	if mir_ctx_a["damage"] == mir_ctx_b["damage"] and mir_valid:
		print("OK  1c: activation roll is deterministic under a fixed event seed")
		passed += 1
	else:
		print("FAIL 1c determinism: %s vs %s" % [mir_ctx_a["damage"], mir_ctx_b["damage"]])
		failed += 1
	...
	if mir_ctx_c["damage"] == 20:
		print("OK  1c: missing event RNG fails loudly and never falls back to raw RNG")
		passed += 1
	else:
		print("FAIL 1c loud-fail: skill activated without an event RNG")
		failed += 1
	...
	if mir_ctx_d["damage"] == 9:
		print("OK  1c: chance-100 activation fires from the event RNG")
		passed += 1
	else:
