...
func _init() -> void:
	...
	if quote.ok and quote.deltas["party:party_gold"] == -30 and game_state.party_gold == 100:
		print("OK  party quote reports spend without mutation"); passed += 1
	else:
		print("FAIL party quote: ok=%s deltas=%s gold=%d" % [quote.ok, quote.deltas, game_state.party_gold]); failed += 1
	...
	if committed.ok and committed.committed and refunded.ok and game_state.party_gold == 100:
		print("OK  party spend commits and recorded-delta refund restores it"); passed += 1
	else:
		print("FAIL party spend/refund: commit=%s refund=%s gold=%d" % [committed.failure_reason, refunded.failure_reason, game_state.party_gold]); failed += 1

	var credit: RefCounted = ledger.commit([CostSpecScript.fixed("party_gold", "party", -25)])
	if credit.ok and game_state.party_gold == 125:
		print("OK  negative fixed cost credits party gold"); passed += 1
	else:
		print("FAIL party credit: %s gold=%d" % [credit.failure_reason, game_state.party_gold]); failed += 1
	...
	if unit_quote.ok and unit_commit.ok and unit.gold == 25:
		print("OK  unit binding quotes and spends UnitData.gold"); passed += 1
	else:
		print("FAIL unit wallet: quote=%s commit=%s gold=%d" % [unit_quote.failure_reason, unit_commit.failure_reason, unit.gold]); failed += 1

	game_state.party_gold = 80
	unit.gold = 50
	var both: RefCounted = ledger.commit([
	...
	if both.ok and game_state.party_gold == 60 and unit.gold == 40:
		print("OK  multi-wallet spend commits atomically"); passed += 1
	else:
		print("FAIL multi-wallet success: %s party=%d unit=%d" % [both.failure_reason, game_state.party_gold, unit.gold]); failed += 1

	game_state.party_gold = 80
	unit.gold = 5
	var failed_both: RefCounted = ledger.commit([
		CostSpecScript.fixed("party_gold", "party", 20),
		CostSpecScript.fixed("unit_gold", "unit", 10, "unit"),
	], {"unit": unit})
	if not failed_both.ok and game_state.party_gold == 80 and unit.gold == 5:
		print("OK  multi-wallet shortfall mutates nothing"); passed += 1
	else:
		print("FAIL atomic shortfall: ok=%s party=%d unit=%d" % [failed_both.ok, game_state.party_gold, unit.gold]); failed += 1
	...
	if not unknown.ok and unknown.missing_resources == ["missing_wallet"]:
		print("OK  unknown resource fails validation"); passed += 1
	else:
		print("FAIL unknown resource: ok=%s missing=%s" % [unknown.ok, unknown.missing_resources]); failed += 1
	...
	if recorded_refund.ok and game_state.party_gold == 10:
		print("OK  refund uses committed delta instead of changed cost"); passed += 1
	else:
