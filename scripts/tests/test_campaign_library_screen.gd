...
func _run() -> void:
	...
	if (
		package_opt.item_count == 1
		and ROOT in package_opt.get_item_text(0)
		and not export_button.disabled
	):
		print("OK  installed packages populate the export picker")
		passed += 1
	else:
		print("FAIL installed package picker")
		failed += 1

	var archive := "user://campaign-library-test.zip"
	screen._on_export_file_selected(archive)
	if (
		FileAccess.file_exists(archive)
		and "Exported %s 1.0" % ROOT in screen.get_node("ResultDialog").dialog_text
	):
		print("OK  export FileDialog callback writes a validated archive")
		passed += 1
	else:
		print("FAIL export callback")
		failed += 1
	...
	if (
		...
		print(
			(
				"FAIL import callback installed=%d changed=%d inert=%s"
				% [installed.size(), signal_state["changed"], identity_after == identity_before]
			)
		)
		failed += 1
	...
	if (
		...
		print("FAIL invalid import handling")
		failed += 1
...
func _write_pack(root_path: String) -> void:
	var files := {
		...
		"data/roster.json":
		{"units": [{"unit_id": "hero", "unit_name": "Hero", "class_id": "fighter"}]},
		"data/class.json":
		{"id": "fighter", "display_name": "Fighter", "base_hp": 20, "base_movement": 5},
	}
