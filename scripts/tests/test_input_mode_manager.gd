...
func _init() -> void:
	...
	_ok(
		(
			InputModeManagerS.resolve_input_mode("auto", "gamepad", desktop, "mouse_keyboard")
			== "gamepad"
		),
		"Auto follows the last detected available mode",
		passed,
		failed
	)
	_ok(
		(
			InputModeManagerS.resolve_input_mode(
				"gamepad", "mouse_keyboard", desktop, "mouse_keyboard"
			)
			== "gamepad"
		),
		"explicit available setting pins the active mode",
		passed,
		failed
	)
	_ok(
		(
			InputModeManagerS.resolve_input_mode(
				"touch", "mouse_keyboard", desktop, "mouse_keyboard"
			)
			== "mouse_keyboard"
		),
		"explicit unavailable setting falls back to detect floor",
		passed,
		failed
	)
	_ok(
		InputModeManagerS.resolve_input_mode("auto", "", mobile, "touch") == "touch",
		"zero-input mobile seed resolves to touch",
		passed,
		failed
	)
	_ok(
		(
			InputModeManagerS.resolve_input_mode("bad", "", desktop, "mouse_keyboard")
			== "mouse_keyboard"
		),
		"invalid stored input mode normalizes to Auto",
		passed,
		failed
	)
	...
	_ok(
		(
			InputModeManagerS.event_to_input_mode(key_ev) == "mouse_keyboard"
			and InputModeManagerS.event_to_input_mode(mouse_ev) == "mouse_keyboard"
		),
		"key and mouse events detect keyboard/mouse mode",
		passed,
		failed
	)
	_ok(
		(
			InputModeManagerS.event_to_input_mode(pad_ev) == "gamepad"
			and InputModeManagerS.event_to_input_mode(stick_ev) == "gamepad"
		),
		"joypad button and real stick movement detect gamepad mode",
		passed,
		failed
	)
	_ok(
		InputModeManagerS.event_to_input_mode(drift_ev) == "",
		"sub-deadzone joypad motion is ignored",
		passed,
		failed
	)
	_ok(
		(
			InputModeManagerS.event_joypad_device(pad_ev) == 4
			and InputModeManagerS.event_joypad_device(stick_ev) == 7
			and InputModeManagerS.event_joypad_device(drift_ev) == -1
		),
		"joypad device tracking ignores drift and records the active pad",
		passed,
		failed
	)
	_ok(
		InputModeManagerS.event_to_input_mode(touch_ev) == "touch",
		"screen touch detects touch mode",
		passed,
		failed
	)
	...
	_ok(
		(
			bool(desktop_available.get("gamepad"))
			and not bool(desktop_available.get("touch"))
			and bool(desktop_available.get("mouse_keyboard"))
		),
		"desktop availability enables gamepad/K&M and grays touch",
		passed,
		failed
	)
	_ok(
		(
			bool(mobile_available.get("gamepad"))
			and bool(mobile_available.get("touch"))
			and not bool(mobile_available.get("mouse_keyboard"))
		),
		"mobile availability keeps supportable gamepad enabled and grays K&M",
		passed,
		failed
	)
	...
	_ok(
		seen == ["gamepad", "mouse_keyboard"],
		"input_mode_changed emits only on real active-mode changes",
		passed,
		failed
	)
	manager.free()
	...
	_ok(
		tracked_button and ignored_drift and tracked_stick,
		"InputModeManager remembers the last joypad that sent real input",
		passed,
		failed
	)
	tracker.free()
	...
	settings.call("save")
	_ok(
		String(live_manager.get("active_input_mode")) == "gamepad" and live_seen == ["gamepad"],
		"SettingsManager save signal refreshes active input mode immediately",
		passed,
		failed
	)
	...
	_ok(
		String(live_manager.get("active_input_mode")) == "mouse_keyboard",
		"controls reset signal re-resolves input mode without an input event",
		passed,
		failed
	)
