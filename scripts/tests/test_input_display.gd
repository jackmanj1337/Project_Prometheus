...
func _init() -> void:
	...
	_check(
		brand_ok,
		"detect_brand classifies Xbox/Nintendo/PlayStation/generic",
	...
	_check(
		(
			...
			and (
				InputDisplay.joypad_button_label(JOY_BUTTON_Y, InputDisplay.Brand.PLAYSTATION)
				== "Triangle"
			)
		),
		"joypad_button_label swaps face buttons per brand",
		"face-label swap wrong"
	)
	...
	_check(
		(
			(
				InputDisplay.joypad_button_label(JOY_BUTTON_LEFT_SHOULDER, InputDisplay.Brand.XBOX)
				== "LB"
			)
			and (
				InputDisplay.joypad_button_label(
					JOY_BUTTON_LEFT_SHOULDER, InputDisplay.Brand.PLAYSTATION
				)
				== "L1"
			)
			and (
				InputDisplay.joypad_button_label(JOY_BUTTON_START, InputDisplay.Brand.NINTENDO)
				== "Start"
			)
		),
		"non-face buttons defer to positional label (PS shoulders relabel)",
		"non-face defer wrong"
	)

	# ---- action_prompt: keyboard mode = the key -------------------------
	var kbd_prompt: String = InputDisplay.action_prompt("confirm", "mouse_keyboard")
	_check(
		kbd_prompt != "" and kbd_prompt == InputDisplay.first_key_for_action("confirm"),
	...
	_check(
		(
			...
			and (
				InputDisplay.action_prompt("confirm", "gamepad", InputDisplay.Brand.PLAYSTATION)
				== "Cross"
			)
			and InputDisplay.action_prompt("more_info", "gamepad", InputDisplay.Brand.XBOX) == "X"
			and (
				InputDisplay.action_prompt("more_info", "gamepad", InputDisplay.Brand.NINTENDO)
				== "Y"
			)
		),
		"action_prompt in gamepad mode returns the brand-correct pad label",
		"gamepad prompt wrong"
	)
	...
	_check(
		(
			...
			and (
				InputDisplay.action_prompt("no_such_action", "gamepad", InputDisplay.Brand.XBOX)
				== ""
			)
		),
		"unbound action yields empty prompt (gamepad falls back to key)",
		"fallback wrong"
	)
	...
	_check(
		(
			...
			and (
				InputDisplay.more_info_hint_for("gamepad", "value", InputDisplay.Brand.XBOX)
				== "Press X for details."
			)
			and (
				InputDisplay.more_info_hint_for("gamepad", "", InputDisplay.Brand.NINTENDO)
				== "Press Y for more info"
			)
			and InputDisplay.more_info_hint_for("touch", "entry") == "Tap any entry for details."
		),
		"more_info_hint_for swaps verb + token per mode",
		"more_info_hint wrong"
	)
