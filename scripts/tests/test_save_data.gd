...
func _init() -> void:
	...
	var defaults_ok: bool = (
		defaulted.format_version == SaveDataScript.FORMAT_VERSION
	...
	var malformed_convoy: RefCounted = (
		SaveDataScript
		. from_dict(
			{
	...
	var convoy_fallback_ok: bool = (
		malformed_convoy.party["convoy"]["entries"].size() == 1
