...
func _init() -> void:
	...
	if first == second and first.size() == 5:
		print("OK  fixed seed reproduces identical event draws: %s" % str(first))
		passed += 1
	else:
		print("FAIL fixed-seed stability: %s vs %s" % [str(first), str(second)])
		failed += 1
	...
	if after_begins == first:
		print("OK  begin_event alone never advances history_hash")
		passed += 1
	else:
		print("FAIL begin_event advanced the chain")
		failed += 1
	...
	if branch_a == branch_a2:
		print("OK  identical history + identical event repeats exactly")
		passed += 1
	else:
		print("FAIL probing repeat differs")
		failed += 1
	...
	if branch_b != branch_a:
		print("OK  different committed history changes later event draws")
		passed += 1
	else:
		print("FAIL butterfly: draws identical despite different history")
		failed += 1
	...
	if rec_draws != other_draws and rec_draws != kind_draws:
		print("OK  event identity (kind + record) isolates draws")
		passed += 1
	else:
		print("FAIL roll transfer across event identities")
		failed += 1

	# ---- record field ORDER is part of the identity (§3) ----
	svc.start_map(FIXED_SEED)
	var ordered: Array[int] = _draws(svc.begin_event("attack", ["a", "b"] as Array[String]), 3)
	var swapped: Array[int] = _draws(svc.begin_event("attack", ["b", "a"] as Array[String]), 3)
	if ordered != swapped:
		print("OK  record field order is part of the event identity")
		passed += 1
	else:
		print("FAIL field order ignored by the seed mix")
		failed += 1
	...
	if saved == restored and replayed == expected_next:
		print("OK  save-dict round trip restores the dice timeline")
		passed += 1
	else:
		print("FAIL save-dict round trip: %s vs %s" % [str(saved), str(restored)])
		failed += 1

	# ---- from_save_dict tolerates missing fields with 0 defaults ----
	svc.from_save_dict({})
	if svc.map_seed == 0 and svc.history_hash == 0:
		print("OK  from_save_dict defaults missing fields to 0")
		passed += 1
	else:
		print("FAIL from_save_dict defaults")
		failed += 1
	...
	if entropy_seed != 0 and svc.history_hash == 0:
		print("OK  start_map() rolls an entropy seed and resets the chain")
		passed += 1
	else:
		print("FAIL entropy start_map: seed=%d hash=%d" % [svc.map_seed, svc.history_hash])
		failed += 1
