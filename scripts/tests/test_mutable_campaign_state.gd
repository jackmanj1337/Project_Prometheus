...
func _init() -> void:
	...
	_check(
		(
			gs.apply_rule_flip("pair_up_enabled", true, "event", "end_of_map")
	...
	_check(
		(
			notice.visible
			and notice.get_node("Panel/Label").text.contains("Pair Up Enabled")
	...
	_check(
		(
			gs.apply_rule_flip("max_skills", 7, "relic", "permanent")
	...
	_check(
		(
			gs.mutable_campaign_state.rule_patches.size() == 2
			and gs.get_effective_campaign_rule("max_skills") == 9
		),
		"patch order is deterministic and last-write wins"
	)

	gs._apply_campaign_rules_dict({"pair_up_enabled": true, "mandated_rules": ["pair_up_enabled"]})
	_check(
		(
			not gs.apply_rule_flip("pair_up_enabled", false, "illegal", "permanent")
	...
	_check(
		(
			restored.restore_mutable_campaign_state(roundtrip.campaign)
			...
			and (
				(
					...
					. carry_forward_facts
					. get("new_fact_from_data", {})
					. get("count", 0)
				)
				== 3
			)
			and (
				restored.mutable_campaign_state.imported_record_ref.get("record_id", "")
				== "record_a"
			)
		),
	...
	_check(
		restored.get_effective_campaign_rule("max_inventory") == 8,
	...
	_check(
		(
			migrated.restore_mutable_campaign_state(legacy.campaign)
