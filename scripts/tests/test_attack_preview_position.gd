...
func _init() -> void:
	...
	if cam.position == before + Vector2(100, 0):
		print("OK  pan_by_pixels mid-map shifts by exact delta")
		passed += 1
	else:
	...
	if cam.position.x == half.x:
		print("OK  pan_by_pixels clamps to left map edge")
		passed += 1
	else:
	...
	if cam.position.x == map_right:
		print("OK  pan_by_pixels clamps to right map edge")
		passed += 1
	else:
	...
	if cam.position.y == half.y:
		print("OK  pan_by_pixels clamps to top map edge")
		passed += 1
	else:
	...
	if cam.position.y == map_bottom:
		print("OK  pan_by_pixels clamps to bottom map edge")
		passed += 1
	else:
	...
	if cam.position == before_zero:
		print("OK  pan_by_pixels zero delta is a no-op")
		passed += 1
	else:
		print("FAIL zero delta moved camera")
		failed += 1

	# ---- pan_by_pixels: null camera / grid -> no crash ---------------
	var bare := CameraController.new()
	bare.pan_by_pixels(Vector2(50, 50))
	print("OK  pan_by_pixels without setup() is a safe no-op")
	passed += 1
	...
	var p_noavoid: Vector2 = AttackPreviewS._place_clear_of(
		Vector2(400, 300), psize, vw, [] as Array[Rect2], m
	)
	if p_noavoid == Vector2(400, 300):
		print("OK  _place_clear_of: no avoid rects leaves an in-view position unchanged")
		passed += 1
	else:
		print("FAIL no-avoid: %s" % p_noavoid)
		failed += 1

	# 2) Overlapping the bottom-left unit-info panel → pushed ABOVE it (no room below).
	var p_bottom: Vector2 = AttackPreviewS._place_clear_of(
		Vector2(8, 600), psize, vw, [unit_info] as Array[Rect2], m
	)
	if (
		p_bottom.y == unit_info.position.y - psize.y - m
		and not Rect2(p_bottom, psize).intersects(unit_info)
	):
		print("OK  _place_clear_of: pushes the panel above the bottom HUD panel")
		passed += 1
	else:
		print("FAIL bottom-avoid: %s (want y=%f)" % [p_bottom, unit_info.position.y - psize.y - m])
		failed += 1

	# 3) Overlapping the top objective panel → pushed BELOW it (no room above).
	var p_top: Vector2 = AttackPreviewS._place_clear_of(
		Vector2(8, 40), psize, vw, [objective] as Array[Rect2], m
	)
	if (
		p_top.y == objective.position.y + objective.size.y + m
		and not Rect2(p_top, psize).intersects(objective)
	):
		print("OK  _place_clear_of: pushes the panel below the top HUD panel")
		passed += 1
	else:
		print(
			"FAIL top-avoid: %s (want y=%f)" % [p_top, objective.position.y + objective.size.y + m]
		)
		failed += 1

	# 4) An off-viewport desired position is clamped back inside.
	var p_clamp: Vector2 = AttackPreviewS._place_clear_of(
		Vector2(5000, 5000), psize, vw, [] as Array[Rect2], m
	)
	if p_clamp.x == vw.x - psize.x - m and p_clamp.y == vw.y - psize.y - m:
		print("OK  _place_clear_of: clamps an off-screen position back into the viewport")
		passed += 1
	else:
		print("FAIL clamp: %s" % p_clamp)
		failed += 1
	...
	var p_defender: Vector2 = AttackPreviewS._place_clear_of(
		Vector2(620, 300), psize, vw, [defender_rect] as Array[Rect2], m
	)
	if not Rect2(p_defender, psize).intersects(defender_rect):
		print("OK  _place_clear_of: pushes the panel clear of the defender tile")
		passed += 1
	else:
	...
	if preview_packed != null:
		...
		if noop_safe and preview._anchor_defender == null:
			...
			print(
				(
					"FAIL reposition/anchor contract: noop_safe=%s anchor=%s"
					% [noop_safe, preview._anchor_defender]
				)
			)
			failed += 1
		preview.queue_free()
	...
	if pv_packed != null:
		...
		if held_transparent and reseeded and alpha_restored and guard_ok:
			print(
				"OK  V027-03a deferred re-pass re-seeds height, restores alpha, guards stale passes"
			)
			passed += 1
		else:
			print(
				(
					"FAIL V027-03a re-pass: transparent=%s h=%f reseeded=%s alpha=%s guard=%s"
		...
		print("FAIL could not load AttackPreview.tscn for V027-03a")
		failed += 1
	...
	for wall_x in [0, 1, 2, WallGrid.MAP_W - 1]:
		for zoom_idx in [5, 6, 7]:  # 2.0× / 3.0× / 4.0× — the high-zoom band
			...
			var panel_sz: Vector2 = Vector2(
				pv2._panel.offset_right - pv2._panel.offset_left,
			...
			var in_view: bool = (
				ppos.x >= 0.0
				and ppos.y >= 0.0
				and ppos.x + panel_sz.x <= view2.x
				and ppos.y + panel_sz.y <= view2.y
			)
			...
			if not (in_view and clear2 and (beside_right or beside_left or pan_flush)):
				...
				print(
					(
						"FAIL V027-03b wall sweep: tile=%s zoom=%.2f panel=%s size=%s def_screen=%s in_view=%s clear=%s right=%s left=%s pan=%s"
						% [
							t,
							cc2.get_zoom(),
							ppos,
	...
	if sweep_failed == 0:
		print(
			"OK  V027-03b wall sweep: panel adjacent + clear of defender on both walls at high zoom"
		)
		passed += 1
	else:
		failed += sweep_failed
	...
	var beside_w: bool = (
		absf(ppos_w.x - (dscr_w.x + tpx_w + margin2)) <= 1.0
	...
	if clear_w and beside_w:
		...
		print(
			(
				"FAIL V027-03b same-frame scroll: panel=%s size=%s def_screen=%s clear=%s beside=%s"
