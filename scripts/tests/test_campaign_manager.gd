...
func _test_map_registry_accessor(dm: Node) -> void:
	...
	_check(
		(
			dm.has_map_registry_entry("map_002_seize")
			and (
				entry.get("map_data_path", "")
				== "res://data/maps/map_002_seize/map_002_seize_data.tres"
			)
			and entry.get("roster_policy", "") == "default_roster"
		),
		"DataManager resolves a map registry entry by id",
		str(entry)
	)
	_check(
		not dm.has_map_registry_entry("map_does_not_exist"),
...
func _test_start_campaign(cm: Node) -> void:
	...
	cm.end_campaign()
	_check(
		not cm.start_campaign("no_such_campaign") and not cm.is_campaign_active(),
...
func _test_launch_resolution(cm: Node) -> void:
	...
	_check(
		...
		"a later node keeps the campaign party instead of reloading the default roster",
		str(later)
	)
	cm.end_campaign()
...
func _test_victory_advances_node(cm: Node, bus: Node) -> void:
	...
	_check(
		...
		"a win records the results payload (node, successor, winner, standings)",
		str(result)
	)
	_check(
		cm.current_node_id == "node_01_rout" and cm.cleared_node_ids.is_empty(),
...
func _test_defeat_parks_on_node(cm: Node, bus: Node) -> void:
	...
	var result: Dictionary = cm.get_pending_result()
	_check(
		not bool(result.get("victory", true)) and not cm.has_pending_victory(),
		"a loss records a non-victory result"
	)
	_check(
		(
			not cm.commit_pending_result()
			and cm.current_node_id == "node_02_seize"
			and cm.cleared_node_ids.is_empty()
		),
...
func _test_campaign_completes_on_terminal_node(cm: Node, bus: Node) -> void:
	...
	var result: Dictionary = cm.get_pending_result()
	_check(
		bool(result.get("campaign_complete", false)) and result.get("next_node_id", "x") == "",
		"a win on the terminal node reports the campaign as complete",
		str(result)
	)
	_check(
		(
...
func _test_single_map_launch_unaffected(cm: Node, bus: Node) -> void:
	...
	_check(
		(
			cm.get_pending_result().is_empty()
...
func _test_campaign_envelope_roundtrip(cm: Node) -> void:
	...
	_check(
		(
			envelope.get("campaign_id", "") == "proving_grounds"
	...
	_check(
		captured.get("cleared_nodes", []) == ["node_01_rout"],
	...
	_check(
		captured.get("vars", {}).get("villages_saved") == 2,
...
func _test_restore_rejects_unresolvable_ids(cm: Node) -> void:
	...
	_check(
		(
			not cm.restore_campaign_state(
				{"campaign_id": "no_such_campaign", "node_id": "node_01_rout"}
			)
			and not cm.is_campaign_active()
		),
	...
	_check(
		(
			not cm.restore_campaign_state(
				{"campaign_id": "proving_grounds", "node_id": "no_such_node"}
			)
			and not cm.is_campaign_active()
		),
...
func _test_restore_of_a_bare_map_save(cm: Node) -> void:
	...
	_check(
		(
			cm.restore_campaign_state({"campaign_id": "", "node_id": "", "cleared_nodes": []})
			and not cm.is_campaign_active()
			and cm.current_node_id == ""
			and cm.cleared_node_ids.is_empty()
		),
...
func _test_pending_result_is_not_position_state(cm: Node, bus: Node) -> void:
	...
	_check(
		(
			envelope.get("node_id", "") == "node_01_rout"
			and envelope.get("cleared_nodes", []) == []
			and not envelope.has("pending_result")
		),
		"the envelope holds the parked node, not the uncommitted win",
		str(envelope)
	)
...
func _test_advance_validates_before_commit(cm: Node, bus: Node, gs: Node) -> void:
	...
	_check(
		not cm.prepare_pending_advance() and cm.has_pending_victory(),
		"an unprepared roster does not consume the pending victory"
	)
	gs.set("roster_ready", true)
	_check(
		(
			cm.prepare_pending_advance()
			and cm.commit_pending_result()
			and cm.current_node_id == "node_02_seize"
			and not cm.has_pending_victory()
		),
...
func _test_branch_requires_explicit_successor(cm: Node, bus: Node) -> void:
	...
	_check(
		(
			bool(result.get("requires_successor_choice", false))
	...
	_check(
		(
			not cm.choose_pending_successor("node_05_defend")
