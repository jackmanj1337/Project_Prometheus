...
func _init() -> void:
	...
	if armsthrift != null and not armsthrift.is_available_for_release() \
			and not synthetic.is_available_for_release() \
			and discipline != null and discipline.is_available_for_release():
		print("OK  metadata excludes authored/synthetic stubs and retains implemented skills")
		passed += 1
	else:
