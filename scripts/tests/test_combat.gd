...
class MockUnit extends Node:
	...
	func battle_speed(_w: Resource = null) -> int:
		...
		if w == null: return data.get("spd")
		return data.get("spd") - maxi(0, w.get("wt") - data.get("strength"))

	func accuracy(_w: Resource = null) -> int:
		var w: Resource = _w if _w else _weapon
		var acc: int = data.get("skl") * 2 + data.get("luk")
		if w: acc += w.get("hit")
		return acc

	func dodge(_w: Resource = null) -> int:
		return battle_speed(_w) * 2 + data.get("luk")

	func crit_rate(_w: Resource = null) -> int:
		var w: Resource = _w if _w else _weapon
		return data.get("skl") / 2 + (w.get("crit") if w else 0)
...
func _make_weapon(p: Dictionary) -> Resource:
	...
	w.hit          = p.get("hit", 80)
	w.crit         = p.get("crit", 0)
	w.range_min    = p.get("range_min", 1)
	w.range_max    = p.get("range_max", 1)
	w.wt           = p.get("wt", 5)
...
func _init() -> void:
	...
	var atk = _make_unit({"name":"Attacker","str":10,"mag":0,"def":5,"res":2,"skl":10,"spd":10,"luk":5,"weapon":iron_sword})
	# Defender uses a bow (no triangle vs sword) to ensure neutral matchup
	var def = _make_unit({"name":"Defender","str":8,"mag":0,"def":4,"res":2,"skl":8,"spd":8,"luk":4,"team":"enemy","tile":Vector2i(1,0),"weapon":iron_bow})
	...
	var def_lance = _make_unit({"name":"Lancedef","def":4,"skl":8,"spd":8,"luk":4,"team":"enemy","tile":Vector2i(1,0),"weapon":iron_lance})
	...
	var atk2 = _make_unit({"name":"Lancer","str":10,"def":5,"skl":10,"spd":10,"luk":5,"weapon":iron_lance})
	var def2 = _make_unit({"name":"Swordsman","str":8,"def":4,"skl":8,"spd":8,"luk":4,"team":"enemy","tile":Vector2i(1,0),"weapon":iron_sword})
	...
	var archer = _make_unit({"name":"Archer","str":8,"def":3,"skl":9,"spd":9,"luk":4,"weapon":iron_bow,"tile":Vector2i(0,2)})
	var pegasus = _make_unit({"name":"Pegasus","str":6,"def":2,"skl":7,"spd":12,"luk":5,"team":"enemy","tile":Vector2i(0,0),"qualities":["flying"]})
	...
	var non_flying = _make_unit({"name":"Soldier","str":6,"def":2,"skl":7,"spd":10,"luk":4,"team":"enemy","tile":Vector2i(0,0)})
	...
	# --- Follow-up ---
	var fast = _make_unit({"name":"Fast","spd":14,"weapon":iron_sword})
	var slow = _make_unit({"name":"Slow","spd":9,"weapon":iron_lance,"tile":Vector2i(1,0),"team":"enemy"})
	...
	var even_a = _make_unit({"name":"EvenA","spd":10,"weapon":iron_sword})
	var even_b = _make_unit({"name":"EvenB","spd":10,"weapon":iron_lance,"tile":Vector2i(1,0),"team":"enemy"})
	...
	# --- Damage minimum 0 ---
	var tanky = _make_unit({"name":"Tank","def":20,"team":"enemy","tile":Vector2i(1,0),"weapon":iron_lance})
	var weak  = _make_unit({"name":"Weak","str":3,"weapon":iron_sword})
