...
func _init() -> void:
	...
	if entry.is_empty():
		...
		quit(1)
		return
	print("OK  registry entry present"); passed += 1

	# Sanity: roster_policy + roster_source point at a fixed roster directory.
	if entry.get("roster_policy", "") == "fixed_test_roster" \
			and String(entry.get("roster_source", "")).ends_with("/"):
		print("OK  registry roster_policy/source shape correct"); passed += 1
	else:
		print("FAIL registry roster shape: policy=%s source=%s" \
	...
	if not ResourceLoader.exists(map_path):
		print("FAIL map_data_path missing: %s" % map_path); failed += 1
		print("Results: %d passed, %d failed" % [passed, failed])
		quit(1); return
	var map_data: Resource = load(map_path)
	if map_data == null:
		print("FAIL map_data did not load: %s" % map_path); failed += 1
	else:
		print("OK  map data .tres loads"); passed += 1

	# Every enemy_placement unit_data_path must resolve and load.
	var placements: Array = map_data.get("enemy_placements")
	if placements.is_empty():
		print("FAIL enemy_placements is empty"); failed += 1
	else:
		var bad_paths := 0
		for p in placements:
		...
		if bad_paths == 0:
			print("OK  all %d enemy unit paths resolve" % placements.size()); passed += 1
		else:
			print("FAIL %d enemy unit paths failed to resolve" % bad_paths); failed += 1
	...
	if dir == null:
		print("FAIL cannot open roster_source: %s" % roster_dir); failed += 1
	else:
		...
		files.sort()
		if files.size() >= 10:
			print("OK  roster has %d .tres files (expected 10+)" % files.size()); passed += 1
		else:
			print("FAIL roster has only %d .tres files" % files.size()); failed += 1
			...
			for f in files:
				...
				if not errors.is_empty():
					...
					for err in errors:
						print("    %s: %s" % [f, err])
			if bad_units == 0:
				print("OK  all roster units pass validate_unit_data"); passed += 1
			else:
