...
func _init() -> void:
	...
	if (
		...
		print(
			"OK  valid fixture installs once by validated identity with optional-media repair report"
		)
		passed += 1
	else:
	...
	if (
		not duplicate.installed
		and _has(duplicate.errors, "already installed")
		and _tree_bytes(expected) == before
	):
		print("OK  existing version rejection preserves every installed byte")
		passed += 1
	else:
		print("FAIL duplicate preservation: %s" % [duplicate.errors])
		failed += 1
	...
	if (
		not invalid.installed
		and not DirAccess.dir_exists_absolute(invalid_store.path_join("installed"))
		and not DirAccess.dir_exists_absolute(invalid_store.path_join(".staging"))
	):
		print("OK  changed/invalid second-pass tree leaves no installed or staging directory")
		passed += 1
	else:
		print("FAIL invalid second pass: %s" % [invalid.errors])
		failed += 1
	...
	for stage in ["extraction", "validation", "promotion"]:
		...
		if (
			...
			print("OK  simulated %s failure cleans all transaction directories" % stage)
			passed += 1
		else:
			print("FAIL %s cleanup: %s" % [stage, failed_result.errors])
			failed += 1
	...
	if not rejected.installed and not DirAccess.dir_exists_absolute(rejected_store):
		print("OK  invalid archive result cannot create installed state")
		passed += 1
	else:
		print("FAIL invalid preflight created state: %s" % [rejected.errors])
		failed += 1
	...
	if (
		not unsafe.installed
		and not DirAccess.dir_exists_absolute(outside)
		and not DirAccess.dir_exists_absolute(unsafe_store.path_join("installed"))
	):
		print("OK  manifest identity cannot escape service-owned final roots")
		passed += 1
	else:
		print("FAIL unsafe identity containment: %s" % [unsafe.errors])
		failed += 1
