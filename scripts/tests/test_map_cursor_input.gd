...
func _init() -> void:
	...
	if inp.decode(_key(KEY_Z))["intent"] == MapCursorInput.Intent.CONFIRM:
		print("OK  decode(Z) → CONFIRM"); passed += 1
	else:
		print("FAIL decode(Z)"); failed += 1

	if inp.decode(_key(KEY_X))["intent"] == MapCursorInput.Intent.CANCEL:
		print("OK  decode(X) → CANCEL"); passed += 1
	else:
		print("FAIL decode(X)"); failed += 1

	if inp.decode(_key(KEY_TAB))["intent"] == MapCursorInput.Intent.NEXT_UNIT:
		print("OK  decode(Tab) → NEXT_UNIT"); passed += 1
	else:
		print("FAIL decode(Tab)"); failed += 1
	...
	if inp.decode(shift_tab)["intent"] == MapCursorInput.Intent.PREV_UNIT:
		print("OK  decode(Shift+Tab) → PREV_UNIT"); passed += 1
	else:
		print("FAIL decode(Shift+Tab)"); failed += 1
	...
	for kc in dir_cases:
		...
		if d["intent"] == MapCursorInput.Intent.MOVE and d["dir"] == dir_cases[kc]:
			print("OK  decode direction → MOVE %s" % str(dir_cases[kc])); passed += 1
		else:
			print("FAIL decode direction: keycode=%d got %s" % [kc, str(d)]); failed += 1

	# ---- decode_key: ESCAPE → CANCEL (cancel binds both X and ESCAPE) ----
	if inp.decode(_key(KEY_ESCAPE))["intent"] == MapCursorInput.Intent.CANCEL:
		print("OK  decode(Escape) → CANCEL"); passed += 1
	else:
		print("FAIL decode(Escape)"); failed += 1
	...
	if dpad["intent"] == MapCursorInput.Intent.MOVE and dpad["dir"] == Vector2i(1, 0):
		print("OK  decode(D-pad Right) → MOVE (1, 0)"); passed += 1
	else:
		print("FAIL decode(D-pad Right): got %s" % str(dpad)); failed += 1
	...
	for row in pad_cases:
		...
		if decoded["intent"] == row[1]:
			print("OK  decode(%s) → expected intent" % row[2]); passed += 1
		else:
			print("FAIL decode(%s): got %s" % [row[2], str(decoded)]); failed += 1
	...
	if still_held and ti._held_dir == Vector2i.ZERO:
		print("OK  note_released clears _held_dir only on a matching key"); passed += 1
	else:
	...
	if ti._held_dir == Vector2i.ZERO:
		print("OK  note_released clears held d-pad direction"); passed += 1
	else:
		print("FAIL d-pad release held_dir=%s" % str(ti._held_dir)); failed += 1
	...
	if poll_first == Vector2i(1, 0) and poll_before == Vector2i.ZERO \
			and poll_repeat == Vector2i(1, 0) and ti._held_dir == Vector2i.ZERO:
		print("OK  poll_direction immediate move, delay, repeat, then clears"); passed += 1
	else:
		print("FAIL poll_direction: first=%s before=%s repeat=%s held=%s" % [
	...
	if dominant == Vector2i(0, 1):
		print("OK  poll_direction uses dominant axis for diagonal vectors"); passed += 1
	else:
		print("FAIL poll_direction dominant axis: %s" % str(dominant)); failed += 1
