...
func _init() -> void:
	...
	if label != null:
		print("OK  DebugLabel node present")
		passed += 1
	else:
		print("FAIL no DebugLabel node")
		failed += 1
	...
	if label != null:
		...
		if label.visible and label.text == "● DEBUG MODE":
			print("OK  banner shown with base text when no aids active")
			passed += 1
		else:
		...
		if label.visible and label.text == "● DEBUG MODE — force-levelup":
			print("OK  banner lists a single active aid")
			passed += 1
		else:
			print("FAIL one-aid banner: text=%q" % label.text)
			failed += 1

		hud._apply_debug_banner(true, two_aids)
		if label.visible and label.text == "● DEBUG MODE — force-levelup, growth+300":
			print("OK  banner joins multiple active aids")
			passed += 1
		else:
			print("FAIL multi-aid banner: text=%q" % label.text)
			failed += 1
		...
		if not label.visible and label.text == "":
			print("OK  banner hidden and text cleared when debug inactive")
			passed += 1
		else:
	...
	if gs != null and bus != null and label != null:
		...
		if label.text.find("force-levelup") != -1:
			print("OK  flag toggle refreshes banner text")
			passed += 1
		else:
			print("FAIL flag toggle did not refresh: text=%q" % label.text)
			failed += 1
		gs.debug_hotseat_override = true
		await process_frame
		if label.text.find("hotseat-all") != -1:
			print("OK  F9 hotseat override appears in the debug banner")
			passed += 1
		else:
			print("FAIL hotseat override missing from banner: text=%q" % label.text)
			failed += 1
...
"""
	...
	if gs_pu != null and reg_pu != null and res_pu != null:
		...
		lead_data.unit_name = "PU Lead"
		lead_data.class_id = "soldier"
		lead_data.unit_id = "hud_lead"
		lead_data.hp = 20
		lead_data.max_hp = 20
		lead.data = lead_data
		var supp: Node = stub_unit_script.new()
		var supp_data := UnitData.new()
		supp_data.unit_name = "PU Support"
		supp_data.class_id = "cavalier"
		supp_data.unit_id = "hud_supp"
		supp_data.hp = 20
		supp_data.max_hp = 20
		supp_data.strength = 10
		supp_data.defense = 10
		supp_data.speed = 9
		supp_data.skill = 8
		supp_data.luck = 4
		supp.data = supp_data
		...
		if lead_shows and supp_hides:
			print(
				"OK  unit-info names the Pair Up support (no stat deltas) on a lead, hides it otherwise (V021-07)"
			)
			passed += 1
		else:
			print(
				(
					"FAIL pair-up indicator: lead_shows=%s supp_hides=%s text=%s"
	...
	c_obj_prot.type = "protect"
	c_obj_prot.unit_ids = ["leader"] as Array[String]
	...
	if (
		lines.size() == 4
		and lines[0] == "Win:"
		and lines[1].find("Rout all hostiles") != -1
		and lines[2] == "Lose:"
		and lines[3].find("Protect: leader") != -1
	):
		print("OK  HUD objective lines: authored rout + protect")
		passed += 1
	else:
		print("FAIL HUD objective lines: %s" % str(lines))
		failed += 1

	# Empty MapData → no lines (panel stays hidden).
	if hud._build_objective_lines(MapData.new()).is_empty():
		print("OK  HUD objective lines: empty MapData → no lines")
		passed += 1
	else:
		print("FAIL empty mapdata produced lines")
		failed += 1
...
"""
	...
	if (
		...
		print("OK  terrain panel starts compact, hint visible")
		passed += 1
	else:
		print(
			(
				"FAIL compact start: desc=%s moves=%s actions=%s hint=%s"
				% [
					hud._terrain_desc.visible,
					hud._terrain_moves.visible,
					hud._terrain_actions.visible,
	...
	if desc_page_ok:
		...
		print(
			(
				"FAIL desc page: desc=%s|%s moves_vis=%s actions=%s|%s hint=%s"
				% [
					hud._terrain_desc.visible,
					hud._terrain_desc.text,
					hud._terrain_moves.visible,
					hud._terrain_actions.visible,
					hud._terrain_actions.text,
					hud._terrain_hint.visible
				]
			)
		)
	...
	if move_page_ok:
		...
		print(
			(
				"FAIL move page: moves=%s|%s desc_vis=%s"
...
"""
	...
	if "—" in hud._terrain_moves.text and "999" not in hud._terrain_moves.text:
		print("OK  wall renders move cost as — instead of 999")
		passed += 1
	else:
		print("FAIL wall move-cost text: %q" % hud._terrain_moves.text)
		failed += 1
	...
	if hud._terrain_coord.text == "Tile (1, 1)":
		print("OK  W6a tile coord (0,0) renders as one-based (1, 1)")
		passed += 1
	else:
		print("FAIL W6a coord at (0,0): %q" % hud._terrain_coord.text)
		failed += 1
	hud._update_terrain(Vector2i(7, 4))
	if hud._terrain_coord.text == "Tile (8, 5)":
		print("OK  W6a tile coord (7,4) renders as one-based (8, 5)")
		passed += 1
	else:
		print("FAIL W6a coord at (7,4): %q" % hud._terrain_coord.text)
		failed += 1
	...
	if not hud._terrain_actions.visible:
		print("OK  actions row hides when no unit is selected")
		passed += 1
	else:
		print("FAIL actions row visible without a selected unit")
		failed += 1
	...
	if (
		...
		print("OK  V021-05 Hidden page hides the whole More Info box")
		passed += 1
	else:
		print("FAIL hidden page: rows still visible")
		failed += 1
	...
	hud.cycle_terrain_more_page()
	var cyc3: bool = (
		hud._terrain_more_page == hud.TERRAIN_PAGE_HIDDEN and not hud._terrain_more_panel.visible
	)
	...
	var pager_synced: bool = (
		hud._terrain_pager.index == hud._terrain_more_page
		and hud._terrain_pager.index == hud.TERRAIN_PAGE_HIDDEN
	)
	if cyc1 and cyc2 and cyc3 and pager_synced:
		print("OK  V021-05 more_info cycles Hidden → Description → Movement → Hidden")
		passed += 1
	else:
		print("FAIL cycle: c1=%s c2=%s c3=%s pager=%s" % [cyc1, cyc2, cyc3, pager_synced])
		failed += 1
	...
	if compact_hit and description_hit and movement_hit:
		...
		print(
			(
				"FAIL terrain click hit-test: compact=%s desc=%s movement=%s"
	...
	if labels_ignore:
		...
		print(
			(
				"FAIL terrain label mouse_filter: desc=%d moves=%d actions=%d (want %d)"
				% [
					hud._terrain_desc.mouse_filter,
					hud._terrain_moves.mouse_filter,
					hud._terrain_actions.mouse_filter,
	...
	if separate_ok and bounded_ok:
		...
		print(
			(
				"FAIL more-info structure: separate=%s bounded=%s min_y=%s mode=%d"
				% [
					separate_ok,
					bounded_ok,
					str(hud._terrain_scroll.custom_minimum_size.y),
	...
	more_unit.queue_free()
	more_grid.queue_free()
	wall_grid.queue_free()
	more_turn.queue_free()
