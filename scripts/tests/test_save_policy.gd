...
func _test_presets() -> void:
	...
	_check(
		(
			SavePolicy.validate(gba, SavePolicy.default_autosave_rules(), 4).is_empty()
			and gba.size() == 2
			and SavePolicy.is_consumed_on_load(gba, "mid_map")
	...
	_check(
		(
			SavePolicy.validate(single, [], 4).is_empty()
			and int(single[0]["count"]) == 1
			and SavePolicy.is_consumed_on_load(single, "mid_map")
	...
	_check(
		(
			SavePolicy.validate(thirty, [], -1).is_empty()
...
func _test_warning() -> void:
	_check(
		not SavePolicy.builder_warnings(SavePolicy.thirty_interchangeable(), 4).is_empty(),
		"durable mid_map plus finite rewind raises the builder warning"
	)
	_check(
		(
			SavePolicy.builder_warnings(SavePolicy.thirty_interchangeable(), -1).is_empty()
...
func _test_campaign_authoring() -> void:
	...
	_check(
		parsed != null and errors.is_empty() and parsed.rule_overrides.get("autosave_rules") == [],
...
func _test_trigger_registry() -> void:
	...
	_check(
		built_in == [true] and custom == [true] and calls == ["battle_end", "author.custom_event"],
...
func _test_rotation_invariant() -> void:
	...
	var manual_pool_full := (
		manager.save_slot("manual_02", _save("Manual two"))
		and manager.save_slot("manual_03", _save("Manual three"))
		and not manager.save_slot("manual_04", _save("Manual four"))
	)
	var mid_consumable := (
		manager.save_slot(SaveManagerScript.MID_MAP_SLOT, _mid_save())
		and manager.should_consume_on_load(SaveManagerScript.MID_MAP_SLOT, SavePolicy.classic_gba())
	)
