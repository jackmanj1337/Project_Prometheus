...
- [x] Filenames, sizes, hashes, Main Menu `v0.5.4`, and the log's BUILD STAMP all
...
- [x] Continue drops you back onto the **same** live map at the exact turn/units
      you left (not a fresh board).
- [x] Play the map to a **victory**. Results resolves normally and offers the real
...
- [x] The win **counts**: the campaign position advances and an autosave is
      written (Continue/Load afterwards resumes at the next node, not the map you
      just won).
- [x] Repeat the suspend→Continue path but this time play to a **defeat**; the
      Defeat screen appears normally (feeds A2).

Successor button wording seen, and node before/after: All good

Note on death during classic mode. Eventually we will swap to a separate unit roster menu and a map based position system where the player has a cursor on the actual map and can select units to trade places.
...
- [x] The map reloads with **all units present** on both sides and the
      **Objectives HUD visible** — not an empty painted map.
- [x] The retried map plays normally from its start (ledger round 0), and can be
...
- [x] The retreated unit re-enters the next map at **full HP** (not 0 / not
      "walking dead").
- [x] The unit that took damage also re-enters at **full HP** (no carried damage
      between maps).
- [x] Contrast — a **suspend & Continue** of a *damaged* board still restores the
      exact mid-map HP (the heal is a fresh-launch step only, never applied to a
      resumed board).

Unit HP entering the next map, and the resumed-board HP: 
...
- [x] The **3rd** manual campaign save still succeeds; the **4th distinct** one is
...
- [x] Saving again at a Prep whose label already exists prompts **"Replace Save?"**
...
- [x] A manual save in a **different** campaign still succeeds while the first
      campaign is at its cap (the budget is **per-campaign**, not global).
- [x] **Load Game** offers a **Delete** on a save (with a confirm prompt); deleting
      one frees a slot so a new save in that campaign succeeds again.

Message text seen at the cap, and per-campaign/Replace behavior: behaves as specified, but eventually we should allow the manual replacement of save files through a save selector so that multiple runs with different rule sets can exist at the same checkpoint and a player can overwrite an chosen old save from the record whenever they are given the opportunity to make a save. We should also take a look at adding labeled and sorted subsections to the load game section that sorts the files by campaign while we wait for the full menu redesign and seperation of data and engine.
...
- [x] With the editor open, pressing **WASD / arrow keys** does **not** scroll or
      move focus on the settings screen behind it.
- [x] If opened over a live map, the **map cursor does not move** and the map does
      not accept input while the editor is open.
- [x] Closing the editor (Done or Cancel) **restores** normal keyboard navigation
      on the settings screen — focus repeat works again immediately.

Screenshot of the open editor, and any leaked movement: Phase marker is locked under the controls and can't be interacted with. 
...
- [x] **Scale Panel −/+** are **disabled (greyed)** until you click a panel frame
      to select it; after selecting a panel, they visibly change its scale.
- [x] **Every** toolbar button responds to a click — **Scale −**, **Scale +**,
      **Reset**, **Done**, **Cancel** — including where a panel frame overlaps the
      toolbar strip (no frame "eats" a toolbar click).
- [x] **Done** saves the layout (reopen Settings → the change persists); **Cancel**
      discards changes back to how the layout was when you opened the editor.
- [x] Drag a panel and confirm it moves; **Reset** returns panels to the authored
      layout.
...
- [x] Menu navigation (Main Menu, Settings, Load Game) works by **d-pad/stick +
      face buttons**; confirm/cancel map to the expected buttons.
- [x] On a map, the **d-pad/stick moves the cursor**, confirm selects, cancel backs
...
- [ ] **Hotplug**: connecting/disconnecting the controller mid-session is handled
      ...
      disconnect.
- [x] While a modal (Results / Defeat / HUD editor) is open, the **d-pad/stick do
      not** drive the map behind it.

Controller model (or `NOT RUN — no controller`): X-box controller could not access prep screen. No repeat on input on defeat screen or victory screen. selector does not cycle either. We should also give a victory screen a `retry` `quit` and `manual save` assuming campaign rules allow for each.
...
- [x] With more rows than fit, the selector shows a **vertical scrollbar** and
      **stays at its authored height** (it does **not** grow taller than the panel
      / off the screen as history grows).
- [x] Scrolling (mouse wheel and **keyboard focus**) moves through the full list;
...
Rough number of rows, and whether it scrolled vs grew: Scrolled at 7 items, but there should be a repeat and some padding so that the player can see a bit ahead of the cursor. The first time the rewind menu is opened, the controler can access it, but after the menu is closed and reopened, the controler and keypad can't access it unless the map is fully reloaded 
...
- [x] The objective reads a plain **Survive N turn(s)** — there is **no**
      "Hold (x, y)" tile requirement.
- [x] **No defeat occurs on any turn up to the survive target** — you are not
      "timed out" before reaching it.
- [x] Surviving **to** the required turn resolves the map as a **victory**.
- [x] Any other map that shows a "Hold (x, y) …" objective reads its coordinates
...
- [x] **Paired escape** — pairing the two escape units and escaping the lead takes
...
- [x] **Manual End Turn** — with more than one unit able to act, End Turn advances
