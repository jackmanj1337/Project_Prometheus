...
- **Windows version:** 11
- **Primary resolution:** Default
- **Monitor size (if smaller than 1920×1080):** 1820x1080
- **Input method:** Keyboard/mouse
- **Build hash verified:** Yes
...
---
# Misc comments
- what is the `Int` in the compact stat view
- Weapons should show their stats on the more info page
- more info page should have a selector that is navigable by directional pads
- character sheet info should include the class with a summary of the class archytpe and any skills or special features.
- 
...
**Tester comments:** Bonus listed as `Pair Up +3 (-)`, consider changing this to `Pair Up +3 (Until Unit Seperation)` or `Pair Up +3` and remove the duration marker  or possibly change it to `Pair Up +3 (From: M950_Cavalier Duration: Until Seperation)`
...
**Tester comments:** Works correctly, but expects some incorrect assumptions. CON and LoS do not appear on the character sheet
...
**Tester comments:** author a stat debufing tonic for checking this next playtest
...
**Tester comments:** good, will probably be changed during the UI pretify pass. We should add a `Support Name` section to the on map character hud
...
**Tester comments:** This is usable but will likely get reworked during the campaign addition
...
**Tester comments:** When moving the camera left, right, or down on zoom levels 3 or 4 the camera jumps around significantly as well as when trying to zoom past 4xm 
...
**Tester comments:** slider moves and seems to read the map zoom level acurately but changing the slider does not change the map. 
...
**Tester comments:** Combat preview overlaps defender at most zoom levels and positions
...
**Tester comments:** what is the difference between borderless and fullscreen?
### E.2 Keep
...
**Tester comments:** TEST NOT RUN no monitor available
...
**Tester comments:** ui scale changes the location of hud and menus, picture available. posibly seperate it  into just menu scaling and let hud scaling be handled by hud layout.
### F.2 No clipping
...
**Tester comments:** Rename scale to say `Scale Panel` and give each panel a bright red outline that turns yellow when it is selected. Also, give each panel some dummy sample text so the user can see what the font size looks like at that scale.
...
**Tester comments:** Slight problem with reset, if the terrain more info page open when the hud is reset then the top of the more info page goes to where the top of the unexpanded panel goes.ffff
...
**Tester comments:** when control switched, expended units were reactivateed and were able to be moved again. otherwise good.
...
**Tester comments:** The objective window incorrectly lists the seize tile  as (15,2)
