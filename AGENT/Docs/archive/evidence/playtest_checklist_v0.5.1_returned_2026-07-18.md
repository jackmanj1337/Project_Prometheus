> **Historical** — the tester's completed v0.5.1 handbook as returned on
> 2026-07-18, preserved verbatim as evidence. Root-caused in
> `AGENT/Code Reviews/playtest_v0.5.1_root_cause_review_2026-07-18.md`; the
> blockers are fixed in `8b77c9d` and await v0.5.2 verification. Do not edit —
> the live handbook is `AGENT/Docs/playtests/playtest_checklist_v0.5.1.md`.
...
- [x] Game reaches Main Menu without a blocking error.
- [x] New Game, Continue, Settings, Load, and Quit behave normally.
- [x] Start Proving Grounds and enter its first tactical map through Prep.
- [x] Move, combat, menus, Suspend & Quit, Continue, Retry, and results still work.
- [x] Keyboard/mouse navigation works; controller works or is marked `NOT RUN`.

Notes: After achieving victory on any map even the old single maps I was unable to continue or return to the main menu. pictures provided
## 2. Rewind selector and coordinates

Complete several blue actions with deliberately different start/end tiles.

- [x] Map Menu shows `Rewind (N)` with the correct remaining charges.
- [x] Selecting Rewind opens a history list rather than rewinding immediately.
- [x] Each row names the activated unit.
- [x] Each row shows accurate `(start x,y) → (end x,y)` coordinates.
- [x] Two same-named units can be distinguished by their coordinates.
- [x] Cancel returns focus to Rewind without moving the map or spending a charge.
- [x] Choosing a row restores the boundary before that activation.
- [ ] The chosen cost is deducted exactly once.
- [x] Inventory, gold, HP, positions, Pair Up, phase, cursor, and threat state match
...
- [x] The final red activation appears with its red unit and coordinates.
...
Notes: rewinding into enemy phase freezes cursor, cannot open menus, inspect units, or zoom camera. Can flip between terrain info pannels. had to close game through windows and relaunch
## 4. Rewind cost modes

Proving Grounds uses the default `per_activation` behavior. The imported
Two-Map Skirmish fixture uses `full_history`.

- [x] In Proving Grounds, older rows show increasing costs as more activations
      are crossed.
- [x] Unaffordable rows are not offered.
- [x] In Two-Map Skirmish, every retained row costs exactly one charge.
- [x] A one-charge selection can reach the oldest retained activation.
- [x] History depth remains bounded by retained data; no nonexistent history appears.
- [x] With zero charges, Rewind is disabled.

Notes and observed costs: how many record should be kept by the two map skirmish?

## 5. Defeat-screen Rewind

- [x] Trigger a defeat after several retained activations.
- [x] Defeat screen Rewind opens the same labelled history selector.
- [x] Coordinates and costs match the Map Menu presentation.
- [x] Selecting a pre-defeat boundary dismisses Defeat and reloads a playable map.
- [x] Cancel returns to the Defeat actions without spending a charge.
...
- [x] Import reports package `two_map_skirmish` version `1.0` successfully.
- [x] New Game lists **Two-Map Skirmish** without automatically starting it.
- [x] It lists **The Crossroads** and **River Pass** as linked chapters.
- [x] The first map loads Alden and Mira as blue units.
- [x] Exactly three red raiders load on the first map.
- [x] Terrain and starting positions match the package data.
...
Notes: When typing export file name x and z keys did not type anything. No units in the two map skirmish has a weapon or proficiency. After suspending two map skirmish map the new game map selector was acting funny and not showing all the available maps until the game was relaunched
...
Notes/status timestamp: `NOT RUN — no fresh v0.5.1 Proving Grounds completion record`
...
Notes: `NOT RUN — no fresh v0.5.1 Proving Grounds completion record`
