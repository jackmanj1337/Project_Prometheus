...
# Playtester Handbook and Checklist - v0.1.5.0
...
## What changed since v0.1.4 (please re-verify these)

Every defect the v0.1.4 pass reported has been fixed. Re-run the listed checks
and confirm they now pass:

- **`iron_axe` log error (9.1)** — the 11,829 `unknown weapon id 'iron_axe'`
  errors are gone; Fighters now carry a real Iron Axe.
- **Pair Up Swap (2.7)** — `Swap` now actually trades the lead and support: the
  new lead takes the on-map tile, the old lead goes off-map; both end their turn.
- **Pair Up bonuses (8.5)** — the support's bonus now reaches the forecast and
  live combat. New: a paired lead's unit-info panel shows a `Paired  +N Str …` line.
- **Allied Rout map end (2.8)** — the map resolves correctly when only a paired
  unit remains; enemies path to it instead of the top-left corner.
- **Promotion modal (8.7 / 8.11)** — the class-choice modal is centered and stays
  fully on-screen.
- **New Game settings (1.2)** — changing Pair Up / Auto Promote and closing the
  panel without Start now remembers the change.
- **Combat preview placement (2.4)** — the preview is kept clear of the HUD panels.

New checks added this pass: **8.3** (Battle Speed now shown in the preview's More
Info) and the **8.5** Pair Up indicator. The log is also much quieter — the M9
skill-stub warnings now appear once each instead of every combat.
...
```powershell
Get-FileHash .\Project_Prometheus_v0.1.5.0_debug.exe -Algorithm SHA256
```
...
- A small grey `v0.1.5.0` label appears at the bottom-right.
...
**Tester comments:** It would be helpful long term to have combat preview display what weapons are being used.
...
**Tester comments:** danger view updated with enemy death ending blue phase, but map is still too small to check map panning memory.
...
`M950_E3_Soldier` without Pair Up or temporary stat modifiers. Open More Info
(`F`) and cycle to the **Damage** field.

**Expected**

- The Damage field's More Info now shows each side's **Battle Speed**, the
  follow-up threshold, and who (if anyone) doubles. (New this build — 8.3.)
...
- The Mage attacks once, not twice, because a difference of 4 is below the
  follow-up threshold of 5.

- [x] **This item works as expected.**

**Tester comments:** defender battle speed does not display when they cannot counter.
...
- After pairing, inspect the Hero (the lead): the unit-info panel shows a
  `Paired  +3 Str +3 Spd +2 Skl +3 Def +1 Lck` line (new this build — 8.5). The
  Pair Up bonus is a combat-only effect, so this line is the readout for it
  outside combat; the support itself stays off-map and is not shown.
...
**Tester comments:** Pairup did not grant any bonuses, and no pairup line showed in lead unit character sheet.
...
- A class that authors a level-1 skill grants it. **Note:** the placeholder
  `Soldier` intentionally has **no** level-1 skill, so reclassing to Soldier
  grants none — this is by design, not a defect. (To see a granted skill,
  reclass to `Mercenary` instead, which grants `armsthrift`.) The General's
  own previously-earned skills are preserved either way.
...
It would help with ease of play to have the class change/promotion stat changes wrap around to a second line instead of a horizontal scroll wheel.
...
Do not report the unchanged behavior below as a v0.1.5.0 regression:
...
Context for what you are testing. This is an early tactical-RPG build; names and
numbers are placeholders and will change.

### Implemented and playable now

- **Grid tactical combat** — move and act; terrain affects movement cost, defense,
  and dodge.
- **Combat math** — damage, hit %, crit %, the weapon triangle (sword/lance/axe and
  the magic triangle), effectiveness vs. unit types (flying / armoured / mounted /
  dragon / beast), and follow-up "doubling" by Battle Speed.
- **Weapon proficiency (WEXP)** and rank-gated equipment.
- **EXP and leveling** in two modes — Random growths and Fixed growths.
- **Promotion** (Master Seal) and **Reclass** (Second Seal), with stat and skill
  changes.
- **Classes and class skills** — a curated set of skills works; many advanced skills
  are placeholders (see "being tested" below).
- **Pair Up** — support stat bonuses, plus the Pair Up / Swap / Separate actions.
...
- **HUD and overlays** — combat preview with More Info, terrain info, objective list,
  and the danger-zone (threat) overlay.
- **Menus** — Main Menu, New Game (per-run rules: Pair Up, Auto Promote, Leveling,
  Permadeath), Settings (audio, movement speed, banners, mouse cursor, and more), and
  **Retry** from the map's starting state.

### Being tested / in validation (focus of this build)

- All of the v0.1.4 fixes listed at the top of this document (Pair Up swap and bonuses,
  promotion modal, allied-Rout map end + AI, New Game settings, combat-preview
  placement, the `iron_axe` data error).
- **2-player local hotseat** (Map 900) is in validation.
- New readouts: **Battle Speed in the combat preview** and the **Pair Up bonus
  indicator** on the unit panel.
- **Heads-up:** skills labelled "implement in M9" are **placeholders** — they appear on
  a unit but do nothing in combat yet. Don't test those for effect; their warning lines
  in `godot.log` are expected.

### Future plans (roughly ordered — subject to change)

1. **Display and accessibility** — map zoom, windowed/fullscreen and resolution
   options, font-size scaling, HUD layout scaling.
2. **Full skill content** — make the placeholder skills actually work.
3. **Status conditions** — poison, silence, berserk, and the rest.
4. **Extra-turn system** — Canto (move again after acting), Dancer/refresh, Galeforce.
5. **Content expansion** — more classes, weapons, items, and skills.
6. **Campaign layer** — pre-battle deployment, a shop, recruiting allied units, and a
   suspend-save.
7. **Laguz / shapeshifters** — deferred until after the campaign layer.
8. **Deeper support system** — Dual Strike / Dual Guard and support conversations —
   deferred.
9. **Online play** — the hotseat foundation is being built to extend to networked play
   (post-1.0).
10. **Platforms** — web playtest channel, Steam Deck verification, and gamepad support.
11. **Pre-1.0 housekeeping** — rename the placeholder (Fire Emblem-derived) names to
