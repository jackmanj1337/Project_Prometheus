...
| 7 | (owner request 2026-08-01) player-facing control | Implemented — a **Viewport Scale** slider in Settings › Display (`SettingsScreen`), value = the factor over 0.5–4.0 by 0.5, driving `SettingsManager.set_content_scale_factor` (normalize → apply → re-reconcile menu scale → save). Uses the Menu-Scale drag policy (preview label live, commit on release, V025-01a). Without this the expand payoff (§A "a bigger display reveals more tiles") was reachable only by hand-editing the cfg. |
