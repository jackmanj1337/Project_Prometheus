...
| `CVR-1..6` | `cover` — Pre-Application Effect-Reassignment Primitive — Open Questions | OPEN | — | `registers/cover_intercept_open_questions_2026-06-26.md` |
...
| `RDR-1..13` | `redirect` — Combat Effect-Redirect Primitive — Open Questions | RESOLVED | 2026-06-26 — full design-walk (session 2026-06-26f); RDR-1..13 settled (RDR-12 `event`-context binding + RDR-13 resource coupling added later same session from composition stress-tests). Remaining items are forward-reqs on other owners (F5/M8 effect event, A5 death-ordering, `[STY-9]` selector, F16/REQ `event` subject, candidate-A/F7 cost-pool model) + a spawned sibling primitive `cover` (`[CVR]`), not open design forks on `redirect` itself. | `registers/redirect_effect_open_questions_2026-06-26.md` |
