...
- [x] The log begins with BUILD STAMP version `0.7.7` and the commit recorded in
  `BUILD_INFO.json`.
- [x] The release executable, debug executable, and Web archive match the supplied
  checksums.

## First-attempt Continue from inactive startup

- [x] Start with the supplied migration pack installed and create either a campaign
  save or a mid-map suspend under version 2.0.0.
- [x] Quit the executable completely, then relaunch it. Do not open Campaign Library
  or New Game before continuing.
- [x] Press Continue exactly once. It restores the saved campaign or battle without
...
- [x] Quit and relaunch once more; first-attempt Continue succeeds again.
- [x] Campaign Library still lists both supplied migration-fixture versions.
