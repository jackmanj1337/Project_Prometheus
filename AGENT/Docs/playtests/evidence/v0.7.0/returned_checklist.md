...
- [x] [UNPROVEN] Every executable matches `SHA256SUMS.txt` (`sha256sum -c SHA256SUMS.txt`).
- [x] [NEW] Startup log BUILD STAMP reads **`version=0.7.0 commit=6cf2c89a`** in EVERY
...
- [ ] [UNPROVEN] The debug executable shows the DEBUG MODE banner and the release executable does not. Same source commit, exported `--export-debug` and `--export-release`.
...
- [x] [UNPROVEN] **Windows are no bigger than they need to be.** Load Game and Campaign Library
...
- [ ] [UNPROVEN] Non-zero safe-area padding; HUD panels attach and clamp.
	- many problems, picture attached. the edit hud controls cover both the phase count and phase indicator and the controls leak off screen at high viewport and the unit info page is not clamped very close to the bottom left corner. and tne hud panels dont stay clamped during the edit screen so a resize can strand them unless edit mode is exited and restarted.
- [x] [UNPROVEN] Contextual action, attack-preview, weapon, item and map menus stay anchored to
  gameplay rather than being forced to screen center.
- [x] [NEW] **Viewport Scale:** changing the setting re-anchors the map and HUD without
...
- [ ] [NEW] **Terrain variants:** tiles introduced by the active pack paint at the correct
  ...
  are exactly what a headless run cannot check. Feeds decision 4.
	  no visually distinct terrain noticed
- [x] [UNPROVEN] Complete a representative map without a crash or stuck modal.

Left click on the map no longer activates units. level up screen also not seen despite levels gained. also noticed red units gaining xp. also report that the case of controls mysteroulsy going dead returned but this time on keyboard. second attempt failed to reproduce any of these except the mouse. recured once during later run for no apparent reason. later also noticed red unit at higher level than they should be once. Also noticed the phase banner does not allways go across the entire screen at 2x viewport
...
- [x] [NEW] **Unit Details stacks below 1024 logical px.** It used to split side-by-side
...
- [ ] [NEW] **Dragging across a class boundary settles once.** Drag the window slowly
  ...
  between layouts.
	  settles once, but the boundary where it changes does not seem to be consistent between resizings
- [x] [NEW] **A class change does not cost you state.** With Unit Details open — a section
...
- [x] [NEW] Same three checks driven from **Settings → Viewport Scale** instead of the
  window: changing the scale while looking at a screen changes its size class too.
looks mostly good, but when opening some menus at nonstandard sizes the background is centered, but the contents run off the right hand side until the screen is resized with the menu open.
## 3. Controller

- [x] [UNPROVEN] Repeat keyboard, mouse and controller navigation across every menu.
- [x] [UNPROVEN] Repeat controller attacks, level-ups, end-turn confirmation and menu transitions.
...
- [x] [UNPROVEN] One list item per press — no run-on movement in dropdowns (the v0.5.6
  successor-dropdown failure).
- [x] [NEW] **While a real pad is in hand:** does joypad button 1 do anything odd on
  ...
  bluetooth xbox controller worked fine.
- [x] [REGRESSION] Hot-plug: connect, disconnect and reconnect the pad mid-session; prompts switch
...
sometime during the playtest a gray bar appeared on the left side near the objective screen, pic included. it later disapeared on a new campaign.
...
- [ ] [NEW] The space key renders with a visible label, and a key the field rejects renders
  disabled with an explanatory tooltip rather than doing nothing.
	  looks good, but key was not disabled at any point I checked.
- [x] [UNPROVEN] The typed value is kept when the dialog is confirmed.

The opening the file menu starts with the focus on the dialog box and typing 'zxwasd' inserts those characters at the begining of the box. the first `escape` click still closes the entire menu and moves it too the new game menu.

picture of the keyboard provided.
...
- [ ] [NEW] With no pack installed, the game starts and says so plainly rather than silently
  offering content.
	  still has the preinstaled packless proving grounds.
- [x] [NEW] Import `campaign-packs/proving_grounds_public.zip` (the archive, not an
  ...
  encounter".
- [x] [NEW] **The board is populated.** Enemies are present, they act on their phase,
...
- [ ] [UNPROVEN] Save, exit, relaunch, continue — the run resumes on the same pack.
	- it appeared so, but the internal pack was saying I was out of save slots the first time I tried to save it after having made 3 saves on the public pack. They should be seperated enough that they shouldn't be able to know how many saves I have on a different pack.
...
- [ ] [NEW] `campaign-packs/proving_grounds_invalid.zip` is **refused at activation** with a
  ...
  invalid campaign loads and can be selected, but when you click start nothing happens.

the prep screen has too much space being taken up by the computer looking rules set. This should at least be line wrapped and scrollable, but we shouldn't put too much effort in as the prep screen is getting a makeover anyway later, but even at 4k the max viewport makes the text cover up all of the unit selection. also look for changes to the combat system recently, the mage was not doing as much damage as it used to but all the stats look about the same, consistent between public and private pack.
...
- [x] [NEW] **Both packs installed at once, sharing content ids.** Import
...
- [x] [NEW] **The four-faction map really has four factions.** Load
...
- [x] [NEW] **Each objective type states its own condition.** The pack ships five distinct
...
- [x] [NEW] **Promotion.** `map_950_promotion_validation` is purpose-built: six level-9
...
- [x] [NEW] **Terrain numbers still behave.** Terrain used to live in **six** engine tables
...
Save slots seem to be getting shared between packs which should not be able to happen.
...
We are not worried about cross version save recovery until v1 stabalizes

## 7. Carry-forward still open

- [x] [REGRESSION] **Filesystem check the logs cannot answer:** confirm NO v0.3.0 resize-trace file
...
- [x] [UNPROVEN] **The whole Godot log directory is attached**, from every executable used.
- [x] [UNPROVEN] Screenshots for anything marked FAIL, plus the Windows resolution sweep.
- [x] [UNPROVEN] Which executable produced each result (debug vs release), and the BUILD STAMP line
  copied from its log.
	  debug used for everything except confirming that the header was not there in the regular release.
