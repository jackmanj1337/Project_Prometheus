...
async function main() {
  ...
  try {
    ...
    const rulesField = Object.entries(observed.rects ?? {}).find(([p]) => p.endsWith("/@Label@2109"));
    if (!rulesField) throw new Error("Rules field not visible in editor snapshot");
    await page.mouse.move(rulesField[1].x + 60, rulesField[1].y + 30);
    await page.waitForTimeout(5000);
    await shot(page, "05c-rules-hover-tooltip");
    await page.mouse.wheel(0, -1500);
    await settle(page);
    observed = await shot(page, "05d-inspector-restored-to-top");
    ...
    await settle(page);
    await shot(page, "08-after-editor-close-menu-scale-restored");
  } finally {
