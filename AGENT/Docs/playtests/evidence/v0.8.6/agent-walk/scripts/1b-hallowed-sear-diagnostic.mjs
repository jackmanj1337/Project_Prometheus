...
async function cycleMoreInfoAndCaptureEach(page, out, label, maxPresses = 2) {
  const shots = [];
...
async function endTurnFromMapMenu(page) {
  ...
  await settle(page);
  console.log("END TURN BEFORE ENTER", JSON.stringify({hud: await readHudLabels(page), map: await mapState(page)}));
  await page.screenshot({path: path.join(OUT, `endturn-before-enter-${stepIndex}.png`)});
  // If a "some units have not acted" confirmation appears it is added straight to the
  // scene root and never shows up in the bridge's screen/rects (see the batch-a note);
  // press Enter unconditionally to match its own on-screen hint. When the Bearer has
  // already Waited this is the ordinary End Turn confirmation instead (same keystroke).
  await page.keyboard.press("Enter");
  await settle(page);
  console.log("END TURN AFTER ENTER", JSON.stringify({hud: await readHudLabels(page), map: await mapState(page)}));
  await page.screenshot({path: path.join(OUT, `endturn-after-enter-${stepIndex}.png`)});
  await page.waitForFunction(
    (name) => {
