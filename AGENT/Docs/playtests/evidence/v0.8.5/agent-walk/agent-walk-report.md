...
Role: dated
Type: playtest
Status: Returned evidence; diagnosis pending
...
The post-hit Unit Details screenshot shows `Hallowed Sear -3 (2 phase)` and Resistance 1. After one manual End Turn and the RED AI phase, the next BLUE phase screenshot (HUD label Turn 3) showed no Bonus condition and Resistance 4. Both units remained at the same tiles and HP. This differs from the checklist's narrated sequence, which expects a `(1 phase)` observation at BLUE Turn 2 and then expiry at BLUE Turn 3. The pre-hit board was Turn 1, while the attack sequence itself was already displaying Turn 2 after the hit; the checklist does not explain this transition. Treat the timing discrepancy as an observed sequence requiring human interpretation/source-state confirmation, not as an isolated proof of a countdown defect.
...
| Hit Details: Hallowed Sear and Resistance -3 | PASS (screenshot) | Screenshot displays `Hallowed Sear -3 (2 phase)` and Res 1; interpreted from the actual screenshot. |
| Turn count after one BLUE/RED cycle | FAIL / sequence mismatch | At Turn 3 after the first manual cycle, condition was already absent and Resistance 4; expected intermediate `(1 phase)`. Turn labels relative to attack need clarification as described above. |
| Suspend & Quit / Continue preserves state | PASS for HP/tiles; condition text comparison partial | HP and tiles match before/after. The condition counter is visible only in screenshots, not machine-readable bridge state; screenshot comparison is retained. |
| Expiry without death/movement | PASS for eventual expiry; exact timing mismatch above | Condition absent by Turn 3; both units survived and remained at the recorded positions. |
...
| 1B countdown, AI, suspend/continue | driven; countdown sequence discrepancy recorded |
...
| 4 internal Chapter 6 interaction | both units alive; forecast stats and suppression explanation PASS; condition breakdown partly verified, count timing discrepancy as above |
...
| 1B — RED AI phase then BLUE Turn 2 shows `(1 phase)` | FAIL / sequence mismatch | Hit action had advanced the HUD from Turn 1 to Turn 2; first manual End Turn plus RED AI reached Turn 3, where condition was already gone. Intermediate `(1 phase)` was not observed. See steps 16–24 in `1B-duration/ch6-land-hit/`. The turn-label sequence is ambiguous; this observation alone does not establish a cause. |
| 1B — Suspend & Quit at `(1 phase)`, Continue, compare condition/HP/tiles | BLOCKED at exact checkpoint; PARTIAL state persistence | No `(1 phase)` checkpoint was reached in the first run. Separate Suspend/Continue replay at post-hit `(2 phase)` preserved HP/tiles exactly: `1B-duration-suspend/ch6-suspend-continue/`. |
| 1B — Expiry and Resistance return, no death/movement | PASS for eventual expiry and unchanged position; timing mismatch above | At Turn 3, no Bonus remained and Resistance was 4; both units remained 13/7 HP at (7,3)/(8,3). Screenshot step 24; bridge state steps 27–29. |
...
| 3 — Ch3 at 900×760, More Info below first two | FAIL (documented layout differs) | Actual layout remains three columns side by side; rows readable/in-frame. `3-forecasts/ch3-900x760/interaction-readout/14-read-the-authored-interaction-rows.png` |
...
| 4 — Land Hallowed hit; Details names Sear and phase count; Resistance breakdown sums to total | PASS for observed hit/breakdown | Screenshot shows Sear −3 with 2 phases and effective Res 1; after expiry Res 4. Evidence under `1B-duration/ch6-land-hit/` (steps 19, 24). Exact next-turn countdown is the mismatch above. |
...
| 7 — Condition name/countdown/−3 explain Resistance | PARTIAL | Details says `Hallowed Sear -3 (2 phase)` and effective Resistance is 1; after expiry it is 4. The arithmetic (4−3=1) is inferable, but checklist wording “Resistance shows −3” is imprecise: −3 appears under Bonus and 1 is the total. |
| 7 — Count goes down once, not twice, across BLUE and RED phases | BLOCKED / sequence mismatch | The `(2 phase)` state was observed after the hit; the next BLUE screen after RED AI had no condition. The stated one-step intermediate was not reached; evidence is in Section 1B. |
...
- The Section 1B sequence needs a native tester or owner to resolve the Turn 1 → Turn 2 transition relative to condition phases. Reported screenshots preserve the observation without assigning a root cause.
