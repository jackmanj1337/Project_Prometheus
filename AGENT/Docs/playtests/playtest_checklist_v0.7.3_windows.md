...
- [ ] Main Menu shows a **Campaign Library** entry of its own, and it is enabled.
- [ ] Campaign Library holds initial focus on a fresh install; keyboard Enter and the
  controller confirm button open it without any prior mouse click.
- [ ] The disabled New Game entry reads **New Game (No Data Packs Installed)** and its
  tooltip explains that a pack must be installed or selected.
- [ ] Campaign Library opens directly from Main Menu — not only through New Game — and
...
- [ ] Escape and the controller cancel button close the library the same way Back does.
