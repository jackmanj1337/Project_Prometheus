...
# v0.7.13 Windows Tester Checklist

This replacement retains the v0.7.12 campaign/save scope and fixes its Settings text
clipping. Return this checklist, requested screenshots, and the complete Godot log
directory. Record observed text and behaviour, not only ticks.

## Build identity

- [ ] Log begins with BUILD STAMP version `0.7.13` and the commit in `BUILD_INFO.json`.
