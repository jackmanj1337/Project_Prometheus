...
> **Returned evidence (2026-07-08).** The tester supplied this filled checklist
> plus separate evidence, archived in `AGENT/Docs/archive/evidence/`:
> - `godot_log_v0.3.0_returned_2026-07-08.log` (final session log) and four
>   per-session logs `godot_v0.3.0_session_2026-07-08T*.log`. All BUILD STAMPs
>   read `version=0.3.0 commit=7b23412 built_at=2026-07-08T07:02:14Z`, matching
>   `playtest_build_v0.3.0.md`.
> - Six §11 windowed-sizing screenshots `v030_resize_*_2026-07-08.png`.
> - `v030_main_menu_2x_overlap_2026-07-08.png` (Main Menu title/button overlap at
>   2.0x Menu Scale).
> - `v030_suspend_resume_support_offmap_2026-07-08.png` (post-Continue state with
>   a support unit rendered off-map).
> Tester environment from the logs: Windows, NVIDIA GeForce RTX 5070 Ti,
> Godot 4.6.stable Compatibility renderer. Controllers: Xbox-layout pad plus a
> PlayStation pad (see §7 comments).
...
**Tester comments:** in the settings menu the controller focus wouldn't go past the visible screen and wouldn't scroll the menu either. The menu also doesn't have any directional repeat which it should. On the new game menu swaping between the map and permadeath setting only takes one keypad or one click of the d-pad or joystick, but permadeath to auto promote one click makes the highlight disapear and the next click makes it reapear. The same occurs between auto promote and leveling, leveling and pair up, and pair up and start, but not between start and back.
...
**Tester comments:** feels good on the map, but in the action menu, equip menu, and the character sheet it felt too fast and occasionally stops weirdly.
...
**Tester comments:** they work, but are too sensitive, tester rebound to to d-pad up and down for remaining test. L3 works well
...
**Tester comments:** Seems to work well. The markers of who is in the special watch list also disapear when the overlay disapears. The overlay also disapears when you try to select an already expended unit, when opening up the pause menu, and when sellecting an unexpended unit. we should work on having both overlays active at the same time. Perhaps we should give each one a texture, or perhaps the threat range gets a strong border line that can be seen through other layers. the temporary view seems to exist on the screen at the same time as the threat overlay, but the movement selector does not.
...
**Tester comments:** input mode selector exists, but it does not seem to block input but does change prompts. Touch will be tested later on a windows machine with a built in touch screen.
...
**Tester comments:** prompts swap live between game pad and keyboard, but when playstation controller is connected first, swaping to xbox mid map does not update prompts and vice versa with xbox connected first. with the playstation control the prompt says "Square" not an actual square icon and the rebind menu did not use different icons at all.

Seperate note, can we make the next unit/prev unit button work for any faction that the cursor is currently on, defaulting to your own faction if not on any unit.  
...
**Tester comments:** After resuming a game all units were visually refreshed but unable to move. the paired up support units were rendered in the placeholder -1,-1 space. suspend and resume while using the debug controls to control red team seems to have corrupted something where the cursor cant be moved and menus can't be opened. The turn counter does not come back acurately the first time but does update to a full count after a full round.
...
**Tester comments:** Not a display issue bit a pathing one made obvious. Can we change the manual pathing method to prefer units follow the path traced out by the cursor up to the limits of their movement, specifically a player can atempt to navigate a path through fog to avoid where they believe an enemy unit to be, or try to walk around a trap even when the shortest path would walk straight over it
...
- [x] **This reads as intended (not a bug).**
...
**Tester comments:** Look at the resizing window test pictures folder. The listed resolution does only seems to update when the actual picture changes size through manual edge dragging, it does not change when using the windows fullscreen button or when draggin on one axis where you just enlarge the black bars. The settings menu did stay centered the entire time though.
...
- [x] **Full feature sweep completed or every skipped row is marked `NOT RUN`
  with a reason.**
...
- [x] **High-attention regressions were stressed, with notes for every failure
  or `NOT RUN` item.**
...
**Tester comments:** Consider adding sensitivity sliders to the triggers and joysticks for the gamepad.
