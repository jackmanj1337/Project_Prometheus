...
> **What this build is.** v0.2.4 is a **focused re-test** of the display and input problems
> you (or a previous tester) reported on v0.2.3. No new features were added — the same game
> as v0.2.3, with eight repairs. **Part I re-checks each repaired item** and is the
> priority: it is the gate that lets us close the display work. **Part II** is a short
> confirmation of the surrounding display surfaces (renderer, resolutions, letterboxing)
> that share the same code. **Part III** is the regression pointer.

## What was repaired since v0.2.3

Each of these maps to a Part I check below.
...
- A monitor that can show **1440p or 4K** is useful for §1.6 and §2.3 (otherwise mark those
  `NOT RUN`)
- Executable: `Project_Prometheus_v0.2.4_debug.exe`
- Expected file size: _see `AGENT/Docs/playtests/playtest_build_v0.2.4.md`._
- Expected SHA-256: _see the build manifest._
- Build manifest: `AGENT/Docs/playtests/playtest_build_v0.2.4.md`
...
```powershell
Get-FileHash .\Project_Prometheus_v0.2.4_debug.exe -Algorithm SHA256
```
...
## 1.1 Menu Scale: 0.5× option and a stable slider (V023-01)

Open **Settings** (`O`) → **Menu Scale**. Drag it across the full range and watch the
**slider handle itself** while you drag. Confirm a **0.5×** value exists at the low end.
Also change scale, close Settings, reopen it, and confirm your choice stuck.

**Expected**

- There is a selectable **0.5×** step (below 0.75×), and the top is still **2.0×**.
- While you change the scale, the **Menu Scale slider row does not slide sideways under your
  cursor** — the control stays put while only the text size changes.
...
with the v0.2.3 return. (Expected, harmless: pre-M9 skill stub warnings such as
...
Do not report the unchanged behavior below as a v0.2.4 regression — these are known and
...
2. Tester comments for every failed or unclear item — **especially §1.4 (weapon row) and
   §1.5 (wheel input)**, which are the two changes we most need confirmed.
...
6. Your overall read on the new **0.5× Menu Scale** readability on your display.
