...
- [x] Filename is exactly `Project_Prometheus_v0.3.2_debug.exe`.
- [ ] SHA-256 matches the build manifest.
- [x] Main Menu shows `v0.3.2`.
...
- [x] Popup selection moves normally.
- [x] The parent screen's focus does not move behind the open popup.
- [x] Closing while a direction is held does not cause a surprise parent-row step.
- [x] Confirm and Cancel affect only the active popup.
...
- [x] Selection reaches every visible information section.
- [x] Selection reaches View Support/View Lead without mouse or shoulder shortcut.
- [x] Confirm opens the paired unit from that selectable entry.
- [x] The sheet scrolls to keep the selected section visible.
- [x] Back remains reachable and closes the sheet.
- [x] No horizontal scrollbar or clipped controls appear at the tested scales.
...
- [x] A tap advances exactly once.
- [x] Held menu movement begins after a noticeable pause and remains controllable.
- [x] No menu skips multiple rows unexpectedly.
...
- [x] Context menus remain anchored correctly after zooming.

Notes: left trigger repeat is just a little fast and the right hand trigger still feels very fast. If we could at least tripple the amount of visual padding around the focus cursor that would also be good.
...
- [x] Readout follows the final width-only size.
- [x] Readout follows the final height-only size.
- [x] Readout converges within one second if a live event is missed.
- [x] The final custom size survives relaunch.
- [x] Maximize shows `Maximized (W x H)` and does not overwrite the saved size.
- [x] Restore returns to the chosen windowed size.
...
- [x] Bright red outlines the complete danger-area union.
- [x] Dark red outlines the watched-threat union.
- [x] Dark red draws over bright red where their edges overlap.
- [x] Both outlines remain readable over unit sprites.
- [x] Movement/path overlays remain readable with the threat display active.
- [x] No stale outline remains after danger mode is turned off or the map changes.

MRD-7 decision:

- [x] Accept `dual_outline` as the default.
- [ ] Reject it; describe the exact visibility/color/width problem below.

Notes: Looks good except when the action menu or the map menu opens the highlight disappears but the outline remains. Can we make the outline stick around during those menus?
