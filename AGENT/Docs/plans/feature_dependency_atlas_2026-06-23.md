...
| F11 | **Skill trigger/effect system** (GDD_05) | 🔧 built, needs effect_ids | combat loop | accessory effects, on-crossing grants, combat arts, gambits, many items | add effect_ids per feature (no new triggers — discipline) · **flagged exception `[RCT-1]` (reactive-reposition, 2026-06-26h):** phase-0 cover-by-swap needs a **NEW pre-resolution reaction trigger** (fires after target-declaration, before the exchange; combat forecast must show the post-reaction defender) — the one genuine break in the no-new-triggers discipline; **needs build-time architecture sign-off** |
...
| F14 | **Author-extensible stat model** (`[STM]`) | ❌ not built | F1, F4 | a Charisma/Charm stat, a Command/Authority proficiency for battalions (`[BAT-6]`/`[STM-2]`), author-defined campaign stats (legacy base stats kept) | ✅ **RATIFIED 2026-06-27d (define-all sweep) — F14 TAKEN (`[STM-3]` path, not the hardcode-Charisma fallback).** *Evolution not rewrite* — add an `extra_stats` dict + a `CampaignRules` stat registry + a `get_effective_stat` fallback and de-hardcode the ~5 stat-list literals; Charisma/Command = author data (`[STM-4]`). `[STM-5]` missing-stat policy. Plan in `registers/extensible_stat_model_open_questions_2026-06-25.md` |
...
| Feature | Size | Needs | Status |
|---|---|---|---|
| Battalions / gambits (#16) | **L** | F11, action flow | **attack-side firmed 2026-06-24j** (`[STY-7]`; gambit = AoE **style** over a battalion-granted source, A1) · **battalion entity firmed 2026-06-25k** (`[BAT-1..13]`; the **attached-augment** pattern — thin entity over reused Pair-Up attach + bonus-resolver + granted-source + rank-helpers; adjutant→Pair-Up, accessory→`[IEQ]`, Emblem = maximal config) · **content/lifecycle `[BAT-14..16]` RESOLVED 2026-06-27d** (BAT-14 always-on flat per-rank block, may touch combat+derived+F14 stats; BAT-15 **two pools** charges+endurance; BAT-16 destruction default **exhausted-repairable** + campaign & per-battalion disband override; host-death → `[DTH]`) |
...
items disperse via the normal chain, no snapshot/restore);
    ...
    **override** disposition: a Key Item is **never `lost`** (always recovered to convoy / re-granted)
    ...
    truly dead); (6) **PvP/skirmish** — no permanent loss; (7) **simultaneous deaths** (AoE wipe; **also `[RDR-8]` redirect/dying-thorns + `[CVR-5]` cover-sacrifice** — owner the **snapshot-then-resolve** rule: apply all HP, collect all at 0, run disposition in a deterministic order, mutual kills both die; a redirect kill credits the **holder** even if dead, for EXP/objectives) —
