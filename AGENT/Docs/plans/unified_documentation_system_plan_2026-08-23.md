...
| 5 | Apply §3 to `playtests`, `Code Reviews`, `design` — 63,632 lines. **Triaged 2026-08-23**, [`uncovered_doc_corpora_triage_2026-08-23.md`](uncovered_doc_corpora_triage_2026-08-23.md): `design` is a keep at 81/81; 129 files / 20,942 lines in the other two are mine-and-delete | **now named, and corrected 2026-08-23:** the two hand-maintained catalogues are the sole live referrer of 61 documents, but only ONE retires. The doc role manifest was deleted at `86931d3b`; `project_control_plane_2026-06-29.md` was measured LIVE — a 129-Track-ID namespace with 2,707 citations, read by checks `[20]`/`[30]` and by `scripts/ci/check_evidence_matrices.py` — and stays. See that document's §6 |
