...
| `campaign.mutable_state.rule_patches[]`, `campaign.mutable_state.carry_forward_facts`, `campaign.mutable_state.imported_record_ref` | `B6-PER-MAP-OVERRIDES`, `B6-CAMPAIGN-STATUS` | `campaign_rules` | Permanent flips append ordered patches; open status facts and imported record identity share one store. | Missing region becomes an empty store. Malformed patch/fact/ref shapes reject restore. | `MutableCampaignState` through `GameState` / `SaveData` | Every ledger checkpoint restores the store, abandoning patches from a discarded future. | Restored in both save kinds. | `campaign_carry`, `old_save_default`, `codec_roundtrip`, `reference_validation` | `v1` | Band 6 mutable campaign state plan Slices 1-2. |
| `campaign.per_map_overrides`, `campaign.active_mid_map_overrides` | `B6-PER-MAP-OVERRIDES` | `campaign_rules` | Node selection seeds the per-map layer; end-of-map flips seed the active layer; commit/cancel clears both. | Empty dictionaries. Unknown rule ids remain data; mandates ignore overlays. | `GameState` open rule resolver via `SaveData` | Restored from each ledger entry. | Restored so an in-progress map keeps its effective rules. | `campaign_carry`, `suspend_restore`, `map_reset`, `old_save_default` | `v1` | Q13 three-layer resolver; mutable campaign state plan Slices 1-2. |
