...
| `map_runtime.rng` | `[CST-8/12]`, RNG-2 | `map_runtime` | Active-map RNG timeline advances on committed non-undoable actions. | Map start seeds from content/campaign; absent old snapshots use legacy Retry path until `B1-SAVECODEC`. | `GameState` snapshot adapter via `SaveCodec` | Restored. | Restored. | `suspend_restore`, `retry_restore`, `codec_roundtrip` | `v1` | RNG design; B1-PKGA Slice 2. |
