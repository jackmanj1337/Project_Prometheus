...
| `header.*` | `[CST-9]` | `campaign` | Slot-preview metadata mirrors save contents and is updated on write. | Derived from campaign fields when absent; missing optional text becomes empty string. | `SaveData` | Not applicable. | Restored. | `codec_roundtrip`, `old_save_default` | `v1` | Campaign save plan. |
| `campaign.campaign_id`, `campaign.node_id`, `campaign.cleared_nodes[]` | `[CST-3]`, `[CNC]` | `campaign` | Created at campaign start; advanced by node-completion effects. | `cleared_nodes` defaults to `[]`; ids must resolve or load fails. | `SaveData` | Restored by snapshot only when the map snapshot includes campaign context. | Restored. | `campaign_carry`, `codec_roundtrip`, `reference_validation` | `v1` | Campaign save plan; `[CNC-1..10]`. |
...
| `campaign.vars`, `map_runtime.vars` | `[TCV-1..6]` | `campaign` / `map_runtime` | Campaign vars carry; map vars seed/reset at map start. | Empty dictionary over registry defaults. | TCV registry adapter via `SaveCodec` | Map vars restored; campaign vars restored only if in snapshot context. | Restored. | `campaign_carry`, `map_reset`, `old_save_default`, `reference_validation` | `v1` | Typed campaign-variable register. |
| `campaign.flags`, `map_runtime.flags` | `[MET-6]`, F6 | `campaign` / `map_runtime` | Campaign flags carry; map flags reset at map start. | Empty set. | MET/TCV adapter via `SaveCodec` | Map flags restored. | Restored. | `campaign_carry`, `map_reset`, `suspend_restore` | `v1` | Map-events register; foundations end-shapes. |
...
| `party.convoy.entries[]` | `[CST]`, `[CNV]`, `[IEQ]` | `party_inventory` | Convoy/party items carry; prep transactions commit immediately. | Empty list. | Inventory adapter via `SaveCodec` | Restored if included in map snapshot. | Restored. | `campaign_carry`, `codec_roundtrip`, `retry_restore` | `v1` | Convoy and item/equipment registers. |
...
| `map_runtime.rng` | `[CST-8/12]`, RNG-2 | `map_runtime` | Active-map RNG timeline advances on committed non-undoable actions. | Map start seeds from content/campaign; absent snapshots leave the live RNG service untouched. | `GameState` Retry snapshot adapter; `SaveData` envelope for persistent/suspend documents. | Restored. | Restored. | `suspend_restore`, `retry_restore`, `codec_roundtrip` | `v1` | RNG design; B1-PKGA Slice 2; B1-SAVECODEC Slice 5. |
| `campaign.rules.rewind_charges` | `[CST-13]`, RNG-3 | `campaign_rules` | Campaign rule stores available rewind charges once rewind is scheduled. | Author default, often `0` for ironman or 3-5 for standard presets. | `CampaignRules` adapter via `SaveCodec` | Restored. | Restored. | `campaign_carry`, `codec_roundtrip` | `dormant_reserve` | RNG design; campaign rules guide. |
