...
| `free_text_dialogue` authored-choice intent provider | M/L | stable dialogue decision-provider seam, authored intent metadata/fixtures, localization, save/replay; remote use also needs authoritative online input | Post-v1 | Optional local rules/embedding/classifier maps human free text only to a currently available authored option ID, with confidence, confirmation, manual fallback, and no authority to generate actions or facts. Full contract: `[DRC]` post-v1 option. |
