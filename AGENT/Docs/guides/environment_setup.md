...
- `export_presets.cfg` — Windows preset used for playtest builds.
  Currently at `v0.3.0.d`; it excludes `AGENT/**`, `scripts/tests/**`, and
...
- `builds/` — `.exe` artifacts. Re-export per
  `playtest_checklist_v0.3.0.d.md` to reproduce.
- `.env` — API keys.
...
godot --headless --path . \
    --export-debug "Project Prometheus v0.3.0.d" \
    builds/Project_Prometheus_v0.3.0.d_debug.exe

# Release build
godot --headless --path . \
    --export-release "Project Prometheus v0.3.0.d" \
    builds/Project_Prometheus_v0.3.0.d.exe
```
