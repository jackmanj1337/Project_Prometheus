...
**Status:** OPEN — design walk captured this session. One **owner scope decision made**
(skirmish = prep-panel now, built against an overworld-ready shape so a later free-roam
overworld is additive, not a rewrite — 2026-07-03f). The rest are OPEN with
recommendations.
...
(class · level/level-range · per-stat ranges · equipment), rolled deterministically from
`seed` (via `RngService`).
...
### [PUG-1] `UnitSpec` model + roll mode  **[OPEN]**
...
- **C — Both, via a `roll_mode` on the spec.**
- **Rec: C.** The editor + skirmish want class-plausible units (B); arena + tuned
  encounters want tight ranges (A). One `roll_mode` field, both backends. Keys off the
  existing class/growth data.

### [PUG-2] Determinism & seed ownership  **[OPEN]**
...
- **Rec: A.** An explicit seed param keeps runtime encounters reproducible (a suspended
  skirmish must re-roll identically) while letting the editor re-roll freely. Depends on
  `RngService` (`B1-PKGA`).

### [PUG-3] The spawn seam — path vs in-memory `UnitData`  **[OPEN — load-bearing]**
...
- **Rec: A.** The single generalization is the smallest change and serves every
...
- **Rec: A.** A first-class `ForceSpec` resource is reusable across skirmish, arena
  ladders, reinforcements, and property-recruitment, and is the thing an overworld
  encounter table references. Consumes `[DIF]`/`[TCV]` for scaling (`[PUG-7]`).

### [PUG-5] `EncounterDef` — the overworld-ready atom  **[SCOPE DECIDED → shape OPEN]**
...
- **Open:** the exact `EncounterDef` schema + whether the trigger surface is an open
  registry (`[EXT]`) so overworld/mission-board/random-event triggers all reuse it without
  engine edits. **Rec:** yes — trigger surfaces are an open registry, `EncounterDef` is the
  shared payload.

### [PUG-6] Skirmish container + map source  **[OPEN]**
...
- **Rec: A.** Reuses the whole prep-panel container + launch spine; the only new surface is
  the `EncounterDef`/`ForceSpec` selection. A later overworld region supplies its own
  `map_source` the same way.

### [PUG-7] Level/count scaling source (auto/random-leveled)  **[OPEN]**
...
- **Rec: an author-selectable scaling rule referencing `[DIF]`/`[TCV]` variables** (an
  open registry of scaling terms, not a hardcoded `match`), defaulting to player-average +
  a difficulty offset. Aligns with `[EXT]` — new scaling rules are data.
...
### [PUG-9] Equipment / inventory generation  **[OPEN]**
- **A — Fixed loadout on the spec;** **B — a weighted equipment table** (by class/tier);
  **C — both.**
- **Rec: C.** A fixed list for tuned encounters, a table for variety; the table keys off
  existing weapon/item data + class proficiency. Degenerates to a fixed list when the table
  has one entry.

### [PUG-10] Consumer map / reuse  **[OPEN — cross-ref]**
...
- **Arena opponents** (`[BEA-5]`, arena plan Slice 2) · **generated recruits** (`[THL-8]`,
  ...
  buy-phase** (`[PVP-3]`).
- **Rec:** one `generate_unit` + one `ForceSpec` + the `[PUG-3]` spawn seam back all of
  them; each consumer differs only in trigger + persistence. This register owns the shared
  contract; the consumers reference it.
...
3. **`ForceSpec` + scaling** (`[PUG-4]`/`[PUG-7]`) — the roster/count/level-scaling
   resource consuming `[DIF]`/`[TCV]`.
4. **`EncounterDef` + the skirmish `[PHB]` panel** (`[PUG-5]`/`[PUG-6]`) — the panel
   trigger fires an `EncounterDef` (generate force → spawn → battle → reward) on an authored
   map pool. Built overworld-ready (trigger surface is a registry).
5. **Editor generate-and-bake** (`[PUG-8]`) — the campaign builder's unit editor calls
   `generate_unit`, shows an editable result, freezes it to a pack `UnitData` on save.
6. **(Later, additive) Overworld** — a free-roam surface whose tiles/regions carry
