...
| `GDD_Manual_Tasks.md` | Implemented (operational) | manual acceptance steps | **move** (DOC-007) | → `AGENT/Docs/guides/manual_test_playbook.md` (content unchanged) | `GDD_00:25`; `testing_guide.md:46,150,175,183`; `GDD_10_Roadmap.md:1337` | 5.2 | New path exists; all 4 live refs updated; feature index manual-coverage cells point to new path |
| ~~`AGENT/GDD/rng_determinism_design_2026-06-11.md`~~ → `AGENT/Docs/design/rng_determinism_design_2026-06-11.md` | Target design (impl. plan) | RNG/save/rewind/online | **DONE (3.1)** move + merge (DOC-010) | Binding rules merged into `GDD_01 §Determinism` + `GDD_02 §Combat`; file moved to Docs, re-scoped as implementation plan, now tracked | none live | 3.1 ✓ | Binding rules in GDD_01/02 ✓; file archived once `RngService` + tests land |
...
| `GDD_07_UI_UX.md` + input/screens companions | Active contracts | **split by ownership** (cross-cutting UI, input/cursor, surfaces) — DONE 2026-07-13 | Band 0 consolidation |
