...
- `export_presets.cfg` — Windows preset used for playtest builds.
  Currently at `v0.2.3`; it excludes `AGENT/**`, `scripts/tests/**`, and
...
- `builds/` — `.exe` artifacts. Re-export per
  `playtest_checklist_v0.2.3.md` to reproduce.
- `.env` — API keys.
...
godot --headless --path . \
    --export-debug "Project Prometheus v0.2.3" \
    builds/Project_Prometheus_v0.2.3_debug.exe

# Release build
godot --headless --path . \
    --export-release "Project Prometheus v0.2.3" \
    builds/Project_Prometheus_v0.2.3.exe
```
