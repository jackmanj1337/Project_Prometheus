...
def check_doc_type_taxonomy() -> None:
    """DSR-1: a front-matter `Type:` must be one of the taxonomy values.

    This is the DoD#2 enforcement for a rule that had none, and the failure it
    catches is silent by construction. gen_docs_index.py sorts INDEX.md by TYPE;
    a value outside the taxonomy matches no section, falls through to the
    filename heuristic, and when that misses too the document is simply **absent
    from INDEX.md** -- with every check still green, because the index
    regenerates consistently and check [18] only compares it to itself.

    That is not hypothetical. On 2026-08-16 twenty-three active documents were
    invisible this way, including seven implementation plans (`Type: implementation
    plan`), five decision records (`Type: decision`, where the taxonomy value is
    `decision-record`), and both UI architecture docs (`Type: design research`).
    Among them were the two plans named as instances of the R1 plan-corpus
    cohesion review -- so the review's own targets were missing from the index it
    would have enumerated them from.

    Two deliberate limits, each with a counter-example in the tree:

    * **Only front matter is read.** `governance/documentation_system_design_2026-06-23.md`
      documents the header format and contains a `Type: register` line inside a
      fenced code block. Scanning the whole file flags that example forever.
    * **A missing `Type:` is not an error.** 92 active documents have no front
      matter at all and index correctly through the filename heuristic. This
      check validates a declared type; it does not mandate one.

    Type is also NOT required to match the containing directory: a handoff
    legitimately lives in `plans/` and a fix plan in `playtests/`.
    """
    docs_root = ROOT / "AGENT/Docs"
    if not docs_root.is_dir():
        return
    valid = set(TYPE_ORDER) if "TYPE_ORDER" in globals() else {
    ...
    for path in sorted(docs_root.rglob("*.md")):
        ...
        try:
            ...
            continue
        for line_no in range(1, end):
            m = re.match(r"^Type:[ \t]*(.+?)\s*$", lines[line_no])
            if not m:
                continue
            declared = m.group(1).strip()
            if declared not in valid:
                _fail("doc-type-taxonomy", path, line_no + 1,
                      f"Type: {declared!r} is not a DSR-1 taxonomy value. INDEX.md is "
                      f"generated by sorting on this field, so an unknown value drops "
                      f"the document out of the index silently -- no error, and every "
                      f"other check stays green. Use one of: "
                      f"{', '.join(sorted(valid))}.")
            break
