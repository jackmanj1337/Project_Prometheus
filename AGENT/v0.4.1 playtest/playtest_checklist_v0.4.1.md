...
---
# v0.4.1 Windows Playtest and Smoke Checklist

Return this completed file and the original `godot.log`. This playtest is the
v0.4.1 release checklist's smoke gate.

This is the single authoritative full-feature rerun for the v0.4 release line. It includes the original Band 2 smoke coverage and every v0.4.1 triage repair.
...
```powershell
Get-FileHash .\Project_Prometheus_v0.4.1_debug.exe -Algorithm SHA256
```

- [x] Filename, byte size, and SHA-256 match the build manifest.
- [x] 59895FFB837B0DA921C21B656EFBEEBCFC755F2A43C8614443651FC10EFD7532 
- [x] Main Menu shows `v0.4.1`.
- [x] The log's BUILD STAMP shows version `0.4.1`, the manifest commit, and a
...
- [x] No release-blocking error appears at the start of `godot.log` after the
      BUILD STAMPdd
...
- [x] Victory resolves normally.
- [x] Victory gold is awarded once through the normal results flow.
- [x] The displayed total changes by the expected amount.
- [x] Reopening menus, changing screens, or waiting does not duplicate the award.

Before: 800  Award: 0  After: 0

Notes: no gold was awareded on the promotion validation map, but 500 gold was awarded on map 001 - Rout
...
## 7. v0.4.1 triage-repair coverage

Use a map with a victory reward and a unit whose More Info prose overflows.

- [x] Results show both `Gold earned` and `Total gold`; Map Menu shows the same total.
- [x] Reopening menus, waiting, and any repeat victory evaluation neither awards nor announces rewards twice.
- [x] With results visible, keyboard taps, held keyboard input, gamepad d-pad/stick, and pointer attempts never move the map cursor behind the overlay.
- [x] More Info descriptions scroll independently with Page Up/Page Down, right-stick vertical, and mouse wheel without moving the selected entry.
- [x] The description scroll hint appears only for overflow, and changing entries resets or clamps the prior offset.
- [x] Unimplemented skills such as Armsthrift are absent from release-facing class choices; implemented skills remain listed.
- [x] No unimplemented-skill warning appears during ordinary release-facing play. If a development-only warning exists in the debug log, record its exact trigger separately and do not confuse it with a release error.
...
- [x] Any failed box has exact reproduction steps: map, unit/UI, action, actual
...
- [ ] Completed `playtest_checklist_v0.4.1.md`.
