# Tactical RPG — Awakening Classes Supplement

> This supplement adds classes from *legacy tactical-RPG source material* not present in the base handbook.
> Stats are adapted to match the handbook's scale and may not match Awakening's exact values.
> All standard handbook rules (promotion at level 20, stat rounding, etc.) apply unless noted.

---

## New Base Classes

---

### Lord
*Noble leaders who fight for their people with sword in hand.*

| Stat | Value |
|------|-------|
| HP | 19 |
| STR | 8 |
| MAG | 0 |
| DEF | 6 |
| RES | 2 |
| SKL | 7 |
| SPD | 7 |
| LUK | 7 |
| MOV | 6 |
| CON | 9 |
| LoS | 4 |

**Proficiencies:** Sword
**Starting Skills:** Charm
**Special Qualities:** —

#### Promotes To:

##### Great Lord
*Lords who have realized their full potential as warriors and leaders.*

| Stat Increase | Value |
|---------------|-------|
| HP | +3 |
| STR | +2 |
| MAG | +0 |
| DEF | +2 |
| RES | +2 |
| SKL | +2 |
| SPD | +2 |
| LUK | +2 |
| MOV | +1 |

**Proficiencies Gained:** Lance
**Promotion Skill:** Rightful King
**Occult Skill:** Aether
**Special Qualities:** —

---

### Tactician
*Strategic battlefield commanders who combine swordplay with elemental magic.*

| Stat | Value |
|------|-------|
| HP | 17 |
| STR | 5 |
| MAG | 6 |
| DEF | 3 |
| RES | 5 |
| SKL | 7 |
| SPD | 6 |
| LUK | 5 |
| MOV | 6 |
| CON | 7 |
| LoS | 4 |

**Proficiencies:** Sword, Anima (Fire or Thunder or Wind — choose one)
**Starting Skills:** Veteran
**Special Qualities:** —

#### Promotes To:

##### Grandmaster
*Tacticians who have mastered both the art of war and arcane power.*

| Stat Increase | Value |
|---------------|-------|
| HP | +2 |
| STR | +2 |
| MAG | +2 |
| DEF | +2 |
| RES | +2 |
| SKL | +2 |
| SPD | +1 |
| LUK | +1 |
| MOV | +1 |

**Proficiencies Gained:** Additional Anima type (Fire, Thunder, or Wind)
**Promotion Skill:** Ignis
**Occult Skill:** Rally Spectrum
**Special Qualities:** —

---

### Dark Mage
*Students of forbidden dark arts whose power comes at a cost.*

> Note: Distinct from the Druid class in the handbook. Dark Mages are a more offensive, glass-cannon dark magic user, where Druids lean defensive.

| Stat | Value |
|------|-------|
| HP | 19 |
| STR | 2 |
| MAG | 8 |
| DEF | 3 |
| RES | 6 |
| SKL | 5 |
| SPD | 4 |
| LUK | 3 |
| MOV | 5 |
| CON | 6 |
| LoS | 4 |

**Proficiencies:** Dark
**Starting Skills:** Anathema
**Special Qualities:** —

#### Promotes To:

##### Sorcerer
*Dark Mages who have plumbed the depths of forbidden power and emerged changed.*

| Stat Increase | Value |
|---------------|-------|
| HP | +2 |
| STR | +1 |
| MAG | +4 |
| DEF | +2 |
| RES | +2 |
| SKL | +2 |
| SPD | +1 |
| LUK | +1 |
| MOV | +1 |

**Proficiencies Gained:** —
**Promotion Skill:** Vengeance
**Occult Skill:** Tomebreaker
**Special Qualities:** —

##### Dark Knight
*Dark mages who took to horseback, blending steel and sorcery.*

| Stat Increase | Value |
|---------------|-------|
| HP | +3 |
| STR | +2 |
| MAG | +3 |
| DEF | +2 |
| RES | +1 |
| SKL | +1 |
| SPD | +2 |
| LUK | +1 |
| MOV | +2 |

**Proficiencies Gained:** Sword
**Promotion Skill:** Lifetaker
**Occult Skill:** Shadowgift
**Special Qualities:** Mounted

> Note: Dark Knight is also an available promotion for the Mage class (handbook p.18) in the Awakening supplement, replacing or supplementing Mage Knight and Sage.

---

### Barbarian
*Wild, brutal axe fighters who rely on raw power and aggression.*

> Note: In Awakening, Barbarian and Brigand are distinct classes. Barbarian is faster and slightly less bulky than Brigand, favoring aggression over raw power.

| Stat | Value |
|------|-------|
| HP | 25 |
| STR | 9 |
| MAG | 0 |
| DEF | 2 |
| RES | 1 |
| SKL | 4 |
| SPD | 6 |
| LUK | 4 |
| MOV | 5 |
| CON | 10 |
| LoS | 4 |

**Proficiencies:** Axe
**Starting Skills:** —
**Special Qualities:** —

**Promotes To:** Berserker or Warrior (see base handbook, p.12 and p.16)

---

### Dancer
*Graceful performers whose art energizes exhausted allies to fight on.*

> Note: Dancers do **not** promote. Their power lies in their unique ability to refresh allies.

| Stat | Value |
|------|-------|
| HP | 15 |
| STR | 2 |
| MAG | 4 |
| DEF | 1 |
| RES | 4 |
| SKL | 5 |
| SPD | 9 |
| LUK | 10 |
| MOV | 6 |
| CON | 5 |
| LoS | 4 |

**Proficiencies:** Sword
**Starting Skills:** Special Dance
**Special Qualities:** —

> **Special Dance (Class Skill):** Once per turn as an action, target 1 adjacent ally who has already acted this turn. That ally may immediately take another full turn. Additionally, the Dancer chooses one stat (STR, SPD, SKL, or LUK); the target gains +4 to that stat until the start of the next turn.
> Special Dance cannot target the same unit twice in a row, and cannot be used on the Dancer itself.

---

### Villager
*Ordinary people with extraordinary potential — untrained, but full of luck and grit.*

| Stat | Value |
|------|-------|
| HP | 16 |
| STR | 5 |
| MAG | 0 |
| DEF | 3 |
| RES | 2 |
| SKL | 5 |
| SPD | 5 |
| LUK | 8 |
| MOV | 6 |
| CON | 7 |
| LoS | 4 |

**Proficiencies:** Lance
**Starting Skills:** Underdog
**Special Qualities:** —

**Promotes To:** GM's discretion — any non-mounted, non-magical base class from the handbook, reflecting the Villager's potential to become anything.

---

## New Laguz-Type Classes

> Taguel and Dragonkin use the same shift mechanics as Laguz (see handbook p.26), with the adjustments noted below.

---

### Taguel
*Beast-tribe shapeshifters who transform into fearsome rabbit-like creatures capable of devastating strikes.*

| Stat | Value |
|------|-------|
| HP | 23 |
| STR | 6 |
| MAG | 0 |
| DEF | 4 |
| RES | 3 |
| SKL | 6 |
| SPD | 9 |
| LUK | 5 |
| MOV | 6 |
| CON | 9 |
| LoS | 5 |

**Proficiencies:** Beaststone (see Beaststone weapon table below)
**Starting Skills:** —
**Laguz Skill (at promotion level):** Beastbane
**Occult Skill:** Stoneborn
**Special Qualities:** Beast\*, Laguz\* (\*animal form only)

| Shift Gauge | Value |
|-------------|-------|
| Starting Shift Gauge | 0 |
| Per Turn (Humanoid) | +8 |
| Per Turn (Animal) | −3 |
| Per Combat (Humanoid) | +7 |
| Per Combat (Animal) | −4 |

#### Beaststone Weapons

| Name | Rank | Mt | Hit | Crit | Range | wEXP |
|------|------|----|-----|------|-------|------|
| Beaststone | E | 4 | 75 | 5 | 1 | 1 |
| Beaststone+ | C | 7 | 80 | 10 | 1 | 2 |
| Beaststone S | S | 11 | 85 | 15 | 1 | 5 |

---

### Dragonkin
*Ancient dragon-blooded beings who channel their heritage through enchanted Dragonstones.*

| Stat | Value |
|------|-------|
| HP | 18 |
| STR | 4 |
| MAG | 9 |
| DEF | 7 |
| RES | 8 |
| SKL | 3 |
| SPD | 4 |
| LUK | 7 |
| MOV | 6 |
| CON | 8 |
| LoS | 5 |

**Proficiencies:** Dragonstone (see Dragonstone weapon table below)
**Starting Skills:** —
**Laguz Skill (at promotion level):** Dragonskin
**Occult Skill:** Wyrmsbane
**Special Qualities:** Dragon\*, Laguz\* (\*animal form only)

| Shift Gauge | Value |
|-------------|-------|
| Starting Shift Gauge | 5 |
| Per Turn (Humanoid) | +6 |
| Per Turn (Animal) | −5 |
| Per Combat (Humanoid) | +8 |
| Per Combat (Animal) | −3 |

#### Dragonstone Weapons

| Name | Rank | Mt | Hit | Crit | Range | wEXP |
|------|------|----|-----|------|-------|------|
| Dragonstone | E | 7 | 70 | 0 | 1 | 1 |
| Dragonstone+ | C | 11 | 75 | 5 | 1 | 2 |
| Divine Dragonstone | S | 16 | 80 | 10 | 1 | 5 |

---

## New Promoted Classes (Awakening Alternatives)

These classes serve as additional promotion options for existing base classes in the handbook.

---

### Bow Knight
*Mounted warriors who blend swordsmanship with deadly long-range archery.*

**Promotes From:** Archer or Mercenary (alternative to Ranger/Sniper or Hero/Sentinel)

| Stat Increase | Value |
|---------------|-------|
| HP | +2 |
| STR | +3 |
| MAG | +0 |
| DEF | +2 |
| RES | +1 |
| SKL | +3 |
| SPD | +2 |
| LUK | +1 |
| MOV | +2 |

**Proficiencies Gained:** Sword (if from Archer) or Bow (if from Mercenary)
**Promotion Skill:** Bowfaire
**Occult Skill:** Rally Skill
**Special Qualities:** Mounted

---

### War Monk / War Cleric
*Clerics who discovered that healing and smiting are not mutually exclusive.*

**Promotes From:** Cleric (alternative to Bishop or Paragon)

| Stat Increase | Value |
|---------------|-------|
| HP | +3 |
| STR | +4 |
| MAG | +0 |
| DEF | +2 |
| RES | +1 |
| SKL | +1 |
| SPD | +1 |
| LUK | +1 |
| MOV | +1 |

**Proficiencies Gained:** Axe (War Monk) or Bow (War Cleric) — choose one at promotion
**Promotion Skill:** Sol
**Occult Skill:** Odd Rhythm
**Special Qualities:** —

---

### Dark Flier
*Pegasus knights who traded divine light for arcane darkness, gaining tremendous magical power.*

**Promotes From:** Pegasus Knight (alternative to Falcoknight or Valkyrie)

| Stat Increase | Value |
|---------------|-------|
| HP | +2 |
| STR | +1 |
| MAG | +4 |
| DEF | +1 |
| RES | +2 |
| SKL | +1 |
| SPD | +2 |
| LUK | +1 |
| MOV | +1 |

**Proficiencies Gained:** Dark
**Promotion Skill:** Galeforce
**Occult Skill:** Rally Magic
**Special Qualities:** Flying, Mounted

---

### Griffon Rider
*Wyvern riders who bonded with griffins instead — swift, resistant, and free of draconic weakness.*

**Promotes From:** Wyvern Rider (alternative to Dracoknight or Wyvern Lord)

| Stat Increase | Value |
|---------------|-------|
| HP | +3 |
| STR | +2 |
| MAG | +0 |
| DEF | +2 |
| RES | +2 |
| SKL | +2 |
| SPD | +3 |
| LUK | +1 |
| MOV | +1 |

**Proficiencies Gained:** Axe
**Promotion Skill:** Deliverer
**Occult Skill:** Lancebreaker
**Special Qualities:** Flying

> Note: Griffon Riders do **not** have the Dragon special quality. They are not subject to Dragon-effective weapons, and do not benefit from Dragon-related skills.

---

### Trickster
*Thieves who learned to mend what they couldn't steal, combining magic utility with acrobatic swordsmanship.*

**Promotes From:** Thief (alternative to Assassin or Rogue)

| Stat Increase | Value |
|---------------|-------|
| HP | +2 |
| STR | +2 |
| MAG | +2 |
| DEF | +1 |
| RES | +2 |
| SKL | +3 |
| SPD | +3 |
| LUK | +2 |
| MOV | +1 |

**Proficiencies Gained:** Staff
**Promotion Skill:** Acrobat
**Occult Skill:** Pass
**Special Qualities:** —

---

### Dread Fighter *(DLC)*
*Elite warriors who pushed past their limits through forbidden training, mastering multiple weapon forms.*

**Promotes From:** Any physical base class (Brigand, Fighter, Mercenary, Myrmidon, Soldier, or Barbarian) at GM discretion

| Stat Increase | Value |
|---------------|-------|
| HP | +3 |
| STR | +3 |
| MAG | +1 |
| DEF | +2 |
| RES | +2 |
| SKL | +3 |
| SPD | +3 |
| LUK | +1 |
| MOV | +1 |

**Proficiencies Gained:** Sword and Axe (if not already held)
**Promotion Skill:** Aggressor
**Occult Skill:** Swordfaire
**Special Qualities:** —

---

### Bride *(DLC)*
*Those who channel love and unity into combat support, excelling at keeping allies fighting.*

**Promotes From:** Any base class at GM discretion

| Stat Increase | Value |
|---------------|-------|
| HP | +2 |
| STR | +2 |
| MAG | +2 |
| DEF | +1 |
| RES | +2 |
| SKL | +2 |
| SPD | +2 |
| LUK | +3 |
| MOV | +1 |

**Proficiencies Gained:** Choose one not already held: Lance, Bow, or Staff
**Promotion Skill:** Charm
**Occult Skill:** Rally Spectrum
**Special Qualities:** —

---

## Optional Rule: Pair Up

*Pair Up is the signature mechanic of Awakening, allowing two units to share a space.*

### How Pair Up Works

Two allied units in the same space form a **Pair**. One unit is the **Lead** (active, acts normally) and one is the **Support** (passive, cannot take independent actions).

- The Support unit is not a separate target — enemies cannot directly attack the Support unit.
- The Lead unit moves for both.
- A unit can join or leave a Pair at the start of their turn before acting. Leaving a Pair places the departing unit in an adjacent empty space.

### Stat Bonuses

While paired, the Lead unit gains a bonus based on the Support unit's class type:

| Support Class Type | Bonus |
|-------------------|-------|
| Physical (STR-primary) | +2 STR |
| Magical (MAG-primary) | +2 MAG |
| Fast (SPD 9+) | +2 SPD |
| Armoured/Mounted | +2 DEF |
| Support (Cleric, Bard, Heron) | +2 LUK and +2 RES |

At GM discretion, a mutual support bond (A-rank or higher) may increase these bonuses by +1.

### Dual Strike

When the Lead unit makes a successful attack: **SKL/2%** chance the Support unit also attacks the target for **50% normal damage**. The Dual Strike does not trigger counterattacks.

### Dual Guard

When the Lead unit is attacked: **SPD/2%** (of the Support unit) chance the Support unit steps in and **nullifies all damage** from that one attack. Dual Guard can trigger at most once per combat.

### Pair Up Skills

The following skills interact with Pair Up (see Awakening Skills Supplement):

| Skill | Effect |
|-------|--------|
| Dual Strike+ | +10% Dual Strike activation chance |
| Dual Guard+ | +10% Dual Guard activation chance |
| Dual Support+ | Stat bonuses granted to Lead unit increase by +1 |
