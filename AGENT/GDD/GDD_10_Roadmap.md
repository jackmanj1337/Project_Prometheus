...
| 2 | `B2-REGISTRY` / `B2-DATAMANAGER-SEAMS` plus `B3-TCV`, `B5-AI-COMPOSITION`, `B3-STAT-REGISTRY` | Continue the open-registry stream. | **Registry foundation + behavior-neutral DataManager phases implemented 2026-07-12:** the shared registries load and validate headlessly; DataManager now has explicit clear/load/validate/report phases and an inert self-contained campaign replace-load seam. Default boot parity and a generated alternate-source fixture are covered. Earlier non-schema AI/stat slices also landed. F1 reserves `ai_awake`, `extra_stats`, and the stat profile id; those consumers still require their serializer/snapshot fixtures. Objective conditions remain sequenced after the typed variable store and predicate system. Next: choose Action/Effect contract groundwork or a save-backed AI/stat consumer. |
