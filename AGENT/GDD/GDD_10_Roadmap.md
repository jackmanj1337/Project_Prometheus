...
| 2 | `B2-ACTION-EFFECT` plus `B3-TCV`, `B5-AI-COMPOSITION`, `B3-STAT-REGISTRY` | Continue the open-registry stream. | **Action/Effect contract groundwork implemented 2026-07-13:** typed request/context/result records, registry-backed handler resolution, pre-mutation schema/subject validation, structured failures, dry-run safety, and the `apply_active_modifier` proof primitive are covered. Existing `stat_buff` items delegate through it without behavior change; a map-event fixture proves cross-domain reuse. Requirement gates and broader domain migrations remain deferred to their owning tracks. F1 reserves `ai_awake`, `extra_stats`, and the stat profile id; those consumers still require serializer/snapshot fixtures. |
