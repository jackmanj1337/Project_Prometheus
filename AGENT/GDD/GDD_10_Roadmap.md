...
| 2 | `B2-ACTION-EFFECT`, `B2-RESOURCE-LEDGER`, `B2-OCCUPANCY`, plus `B3-TCV`, `B5-AI-COMPOSITION`, `B3-STAT-REGISTRY` | Continue the open-registry stream. | **First three Band 2 consumer contracts implemented 2026-07-13:** actions have typed validation/dry-run; fixed party/unit wallets have atomic transactions; map-start placement now uses registry-dispatched occupancy transactions with deterministic fallback and node-free validation. Requirement gates, formulas, resource pools, broader placement consumers, and persistent delay remain deferred. F1 reserves `ai_awake`, `extra_stats`, and the stat profile id; those consumers still require serializer/snapshot fixtures. |
