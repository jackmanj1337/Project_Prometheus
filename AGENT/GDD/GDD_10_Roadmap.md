...
| 2 | `B2-ACTION-EFFECT`, `B2-RESOURCE-LEDGER`, plus `B3-TCV`, `B5-AI-COMPOSITION`, `B3-STAT-REGISTRY` | Continue the open-registry stream. | **Action/Effect and Resource Ledger contract groundwork implemented 2026-07-13:** actions have typed validation/dry-run and the shared modifier proof primitive; fixed registered party/unit wallets now have quote, transient reserve, atomic commit, and recorded-delta refund. Victory gold delegates through the ledger without changing item rewards. Requirement gates, formulas, resource pools, and broader consumers remain deferred. F1 reserves `ai_awake`, `extra_stats`, and the stat profile id; those consumers still require serializer/snapshot fixtures. |
