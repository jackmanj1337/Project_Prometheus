...
| 2 | `B1-LEDGER` Phase 2 | **Planned:** the two-tier decaying ledger (fine per-activation + coarse per-round, data-driven prune), the `undo_activations` / `undo_rounds` budgets on `CampaignRules`, and Retry re-expressed as `restore_history(0)` — scrapping the separate `take_map_snapshot`/`restore_map_snapshot` party-only path. | Prune keeps `(last A activations) UNION (last R round-starts)`, modeled as data not a mode `match`. This is the deliberate scrap-old-code step. |
